...
%     javaaddpath('/home2/s170480/matlab/extern/bioformats/bioformats_package.jar','-end')
    iCell = allCellsSet{i};
%     disp(['Reading movieData for cell ID key: ' iCell.key]);
    movieFileOut = [masterMovieDir filesep iCell.key '_CellX.ome.tiff'];
    movieFileOutMD = [masterMovieDir filesep iCell.key '_CellX.mat'];
    disp(['Reading MD : ' movieFileOutMD]);
...
%     disp('Loading with MovieData BF reader')
    MD = MovieData.load(movieFileOutMD);
%     MD.sanityCheck();
%     MD.save();
...
save([omeTiffDir filesep 'allCellDataMD_2.mat'], 'allCellsSet');
disp('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%');
...
% ML.sanityCheck();
ML.save();
save([omeTiffDir filesep 'allCellDataML_2.mat'], 'ML', 'allCellsSet');
