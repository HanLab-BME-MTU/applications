...
function [] = pcMetaLBPWell_mixMetEffLabels()
...
for iTest = 1 : nTest 
    ...
    for iScale = 1 : nScales % resolution (1- maximal)
        ...
        if ~exist(allFeatsFname,'file') || always                                                
            ...
            if ~exist([outDnameScaleAll 'allMaps.mat'],'file')
                [allMapsMetEff] = getLbpDistributionsMixed(dataAll,metEffAll,generalStats);
                [allMapsSource] = getLbpDistributionsMixed(dataAll,sourceAll,generalStats);
