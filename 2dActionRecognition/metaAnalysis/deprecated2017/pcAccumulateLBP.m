...
accLbpPrefix = [analysisDirname 'metaAnalysis/accumulatedLBP_'];
...
accLbpFnameOld = [accLbpPrefix num2str(iScale) '.mat'];

if exist(accLbpFnameOld,'file')
   
else
    %% init
    accLbpFov.accLbpFov = [];
    
    accLbpFov.cellTypes.strs = [];
    ...
    for iCellType = 1 : nCellTypes
        accLbpFov.cellTypes.accLbpFov{iCellType} = [];
    end
    ...
    for itask = 1 : metaData.tasks.N
        ...
        %% Actual accumulation
        for iScale = 1 : nScales
            accLbpFov.accLbpFov = [accLbpFov.accLbpFov,accumulatedFovPyramidLBP{iScale}]; %#ok<USENS> % lbpData.fov{icell}.lbp(curT,:)
            
            if strcmp(curSource,'Tumors')
                accLbpFov.tumorAll.accLbpFov = [accLbpFov.tumorAll.accLbpFov,accumulatedFovPyramidLBP{iScale}];
            else if strcmp(curSource,'CellLines')
                    accLbpFov.cellLines.accLbpFov = [accLbpFov.cellLines.accLbpFov,accumulatedFovPyramidLBP{iScale}];
                else if strcmp(curSource,'Melanocytes')
                        accLbpFov.melanocytes.accLbpFov = [accLbpFov.melanocytes.accLbpFov,accumulatedFovPyramidLBP{iScale}];
                    end
                end
            end
            ...
            if ~isnan(curMetEff)
                if curMetEff
                    accLbpFov.tumorHigh.accLbpFov = [accLbpFov.tumorHigh.accLbpFov,accumulatedFovPyramidLBP{iScale}];
                else
                    accLbpFov.tumorLow.accLbpFov = [accLbpFov.tumorLow.accLbpFov,accumulatedFovPyramidLBP{iScale}];
                end
            end
            
            % Cell Type
            accLbpFov.cellTypes.accLbpFov{cellTypeInd} = [accLbpFov.cellTypes.accLbpFov{cellTypeInd},accumulatedFovPyramidLBP{iScale}];
...
fprintf('Accumulated variance %d:\n',iScale);
