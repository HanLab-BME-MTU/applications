function [gapCloseParam,costMatrices, ...
    kalmanFunctions, ...
    probDim,verbose ...
    ]=tracker_paramPhillipe(searchRadius)
...
% Assaf note on minSearchRadius - the search radius is dynamically estimated based on
% previous motion, minSearchRadius gives us a minimum search radius. 2nd
% alternative would be to increasin the brownStdMult used to scale the
% estimated std on the noise of the trajectory
parameters.minSearchRadius = searchRadius; %minimum allowed search radius. The search radius is calculated on the spot in the code given a feature's motion parameters. If it happens to be smaller than this minimum, it will be increased to the minimum.
parameters.maxSearchRadius = searchRadius; %maximum allowed search radius. Again, if a feature's calculated search radius is larger than this maximum, it will be reduced to this maximum.
...
parameters.minSearchRadius = searchRadius; %minimum allowed search radius.
parameters.maxSearchRadius = searchRadius; %maximum allowed search radius.
