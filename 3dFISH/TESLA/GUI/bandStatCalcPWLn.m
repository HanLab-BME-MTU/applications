function [bandStat, shortThreshPos, marker] = bandStatCalcPWLn(handles, fIM, shortThresh)
...
for num = 1:marker.num-1
    regPara = G(num:num+1,:)\marker.sizeLn(num:num+1);
...
if shortThresh >= marker.size(1)
    shortThreshPos = (log(shortThresh)-para(1).intercept)/para(1).slope;
else if shortThresh < marker.size(marker.num)
        shortThreshPos = (log(shortThresh)-para(marker.num).intercept)/para(marker.num).slope;
    else
        for num = 1:marker.num-1
            if shortThresh < marker.size(num) && shortThresh >= marker.size(num+1)
                shortThreshPos = (log(shortThresh)-para(num).intercept)/para(num).slope;
            end
        end
    end
end
...
for q=1:size(fIM,2)
    for p=1:size(fIM,1)
        if bandMap(p,q) == 1 && (q < ladder.left || q > ladder.right)
            ...
            if p <= marker.pos(1)
                bandStat(bandIndex).bandSize = exp(para(1).slope*p+para(1).intercept);
            else if p > marker.pos(marker.num)
                    bandStat(bandIndex).bandSize = exp(para(marker.num-1).slope*p+para(marker.num-1).intercept);
                else
                    for num = 1:marker.num-1
                        if p > marker.pos(num) && p <= marker.pos(num+1)
                            bandStat(bandIndex).bandSize = exp(para(num).slope*p+para(num).intercept);
                        end
                    end
                end
            end
