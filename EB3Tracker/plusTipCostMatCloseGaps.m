...
%         % vectors at ends of endsToConsider tracks
...
% 
%         % vectors at starts of startsToConsider tracks
...
%         Dmag = sqrt(Dx.^2 + Dy.^2);
%         Dz = tGap.*ones(size(Dx));
%         DmagWithTime = sqrt(Dx.^2 + Dy.^2 + Dz.^2);
% 
%         % cos angle between track 1 end and track 2 start
...
% 
%         % find all pairs of ends and starts where the start is in the
%         % forward cone of the end, the vectors point the same general way,
%         % and the displacement vector between end and start is shorter than
%         % the forward cutoff distance for current time gap
%         pointSameWay   = cosEF_SF>0;
%         startInFwdCone = cosEF_D>=cos(maxFAngle);
% 
%         [endIdxFWD,startIdxFWD] = find(pointSameWay & startInFwdCone & Dmag<=cutoffDistFwd);
%         [tempIdxFWD] = find(pointSameWay & startInFwdCone & Dmag<=cutoffDistFwd);
%         if ~isempty(endIdxFWD)
%             indx1(linkCount:linkCount+length(endIdxFWD)-1) = endsToConsider(endIdxFWD);
%             indx2(linkCount:linkCount+length(endIdxFWD)-1) = startsToConsider(startIdxFWD);
%             %cost(linkCount:linkCount+length(endIdxFWD)-1)  = 2-(abs(cosEF_SF(tempIdxFWD))+abs(cosEF_D(tempIdxFWD)));
%             %cost(linkCount:linkCount+length(endIdxFWD)-1)  = Dmag(tempIdxFWD).*(2-(abs(cosEF_SF(tempIdxFWD))+abs(cosEF_D(tempIdxFWD))));
%             cost(linkCount:linkCount+length(endIdxFWD)-1)  = abs(cosEF_SF(tempIdxFWD))+DmagWithTime(tempIdxFWD);
%             type(linkCount:linkCount+length(endIdxFWD)-1)  = 2*ones(length(endIdxFWD),1);
%         end
% 
%         linkCount = linkCount+length(endIdxFWD);
% 
     end


    % NOW LOOK FOR BACKWARD LINKS
...
% 
%         
%         % for all the tracks ending in frame i,
%         % [endIndex, indexOfPtAlongTrack, x, y, vel, reverseCumVel]
%         tempEnd=endTrackInfo;
%         % remove all points along tracks which extend beyond the backward
%         % cutoff distance
%         tempEnd(tempEnd(:,6)>cutoffDistBwd,:)=[];
% 
%         % xy-coordinates of startsToConsider
%         startXY=[trackStartPxyVxy(startsToConsider,1),trackStartPxyVxy(startsToConsider,2)];
% 
%         % points along end track vs. starting point of start track distance
%         D=createDistanceMatrix(tempEnd(:,3:4),startXY);
%         [sD,idx]=sort(D,1);
%         startIdx=find(sD(1,:)<d1Max)'; % index of startsToConsider tracks within d1Max of an endToConsdier
%         endIdx=idx(1,startIdx)'; % index of tempEnd row corresponding to startIdx
%         d=sD(1,startIdx)'; % distance of start pt to NN pt on end track
%         Dmag=tempEnd(endIdx,6)+d; % distance from picked pt to track end along track, plus dist from start pt to NN on end
% 
%         % get list of first coordinates from tracks starting in j frame
%         SFx=trackStartPxyVxy(startsToConsider(startIdx),3);
%         SFy=trackStartPxyVxy(startsToConsider(startIdx),4);
%         SFmag = sqrt(SFx.^2 + SFy.^2);
% 
%         EFx=zeros(length(endIdx),1);
%         EFy=zeros(length(endIdx),1);
%         for i=1:length(endIdx)
%             % check whether next point in tempEnd is from the same track, if
%             % yes then forward vector points toward it, if not then forward
%             % vector points to the next point.
%             b=min(2*n,find(tempEnd(:,1)==repmat(tempEnd(endIdx(i),1),[size(tempEnd,1) 1]),1,'last')-endIdx(i));
%             if b<n
%                 % close to the track end, just use that value
%                 EFx(i)=trackEndPxyVxy(endsToConsider(tempEnd(endIdx(i),1)),3);
%                 EFy(i)=trackEndPxyVxy(endsToConsider(tempEnd(endIdx(i),1)),4);
%             else 
%                 % get vector pointing to next end-ward point
%                 EFx(i)=mean(diff(tempEnd(endIdx(i):endIdx(i)+n,3)));
%                 EFy(i)=mean(diff(tempEnd(endIdx(i):endIdx(i)+n,4)));
%             end
%         end
%         EFmag = sqrt(EFx.^2 + EFy.^2);
% 
...
%         pointSameWay = cosEF_SF>0;
% 
%         endIdxTemp=endsToConsider(tempEnd(endIdx,1));
%         startIdxTemp=startsToConsider(startIdx,1);
% 
%         nPossBwd=length(endIdxTemp);
%         notInFwdCone=logical(ones(nPossBwd,1));
%         for i=1:nPossBwd
%             test=repmat([endIdxTemp(i) startIdxTemp(i)],[length(indx1),1]);
%             matchIdx=find(sum(test==[indx1 indx2],2)==2);
%             if ~isempty(matchIdx)
%                 notInFwdCone(i)=0;
%             end
%         end
%         
% 
%         keepList=pointSameWay & notInFwdCone;
% 
%         endIdxBWD=endIdxTemp(keepList);
%         startIdxBWD=startIdxTemp(keepList);
% 
%         if ~isempty(endIdxBWD)
%             indx1(linkCount:linkCount+length(endIdxBWD)-1) = endIdxBWD;
%             indx2(linkCount:linkCount+length(endIdxBWD)-1) = startIdxBWD;
%             %cost(linkCount:linkCount+length(endIdxBWD)-1)  = 1-abs(cosEF_SF(keepList));
%             cost(linkCount:linkCount+length(endIdxBWD)-1)  = Dmag(keepList).*(2-(abs(cosEF_SF(keepList))+abs(1-d(keepList)./d1Max)));
%             type(linkCount:linkCount+length(endIdxBWD)-1)  = 3*ones(sum(keepList),1);
%         end
% 
%         linkCount = linkCount+length(endIdxBWD);
% 
% 
% 
