function [handles] = wyssFileAlignment2(List,List2,name,varargin)
...
editX = uicontrol(hp1,'Style','edit','String','0.00',...
                 'Position',[30,400,40,30],'ButtonDownFcn',@edAdj);             
textY = uicontrol(hp1,'Style','text','String','y = ', ...
                 'Position',[10,360,20,30]);             
editY = uicontrol(hp1,'Style','edit','String','0.00', ...
                 'Position',[30,360,40,30],'ButtonDownFcn',@edAdj);                          
s = uicontrol(hp2,'Style','slider','Min',1,'Max',numel(List), ...
              'Value',1,'Position',[10,10,150,30], ...
              'Callback',{@slideradjust}, 'SliderStep',[1/(numel(List)-1),10/(numel(List)-1)]);
...
shiftad = struct('a',a,'s',s,'xs',xslide,'ys',yslide,'shift',[0,0],'text',[editX,editY]);
...
m = max(log(tmp(:)));
imshow(log(tmp),[0,m*0.7]);
%colormap('hot');
colormap('pink');   
end

function edAdj(hObject, eventData, handles)
