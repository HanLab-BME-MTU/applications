...
%            command line (see VARARGIN)
    ...
    if ~isdeployed
        historyFile = fullfile( fileparts(mfilename('fullpath')),'CellSegmentationQualityAnnotatorHistory.mat' );
    ...
    if strcmp(PARAMETERS.mode, 'analysis')
        ...
        if isdeployed
            regionMergingModelFile = handles.defaultParameters.nucleiSegmentation.regionMergingModelFile
        end
    else
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    handles.data = [];
        ...
        if ~isfield(annotationData.metadata, 'pixelSize') % check if old metadata struct
            ...
            metadata.numChannels = numel(annotationData.imageData);
            
            metadata.channelIdNuclei = 1;
            
            metadata.channelNames = repmat({'Unknown'}, 1, metadata.numChannels);
            metadata.channelNames{channelIdNuclei} = 'Nuclei';
