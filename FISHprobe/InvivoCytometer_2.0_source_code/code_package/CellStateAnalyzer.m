...
%            command line (see VARARGIN)
    ...
    if ~isdeployed
        historyFile = fullfile( fileparts( mfilename( 'fullpath' ) ), 'CellStateAnalyzerHistory.mat' );
    ...
    if ~isdeployed
        ...
        f = rdir( fullfile(ctfroot, '**', 'G1_S_G2_M.model') );
    end
    ...
    if isdeployed
        cellCycleStateIdentificationModelDir = handles.defaultParameters.classification.cellCycleStateIdentificationModelDir
    end
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    set( handles.CellStateAnalyzer, 'Name', sprintf( 'Cell State Analyzer - %s', handles.data.metadata.dataFilePath{1} ) );
...
function RunAnalysis(hObject, handles)
    ...
    [handles.data.imLabelCellSeg, ...
     handles.data.imCellSeedPoints] = segmentCellsInIntravitalData( handles.data.imageData{1}, ...
                                                                    ...
                                                                    'thresholdingAlgorithm', handles.parameters.segmentation.thresholding, ...
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    % save data
    [pathstr, name, ext] = fileparts( handles.data.metadata.dataFilePath{1} );
    ...
    summary_fid = fopen( fullfile(outputDir, 'cellStateAnalysisSummary.txt'), 'w' );    
        ...
        fprintf( summary_fid, '\n\tTwo-photon Histone (CFP) data file -- %s\n', handles.data.metadata.dataFilePath{1} );
    
        fprintf( summary_fid, '\n\tConfocal FUCCI data file -- %s\n', handles.data.metadata.dataFilePath{2} );
        ...
        cellPatternStats = { 'Histone (CFP) data', handles.data.metadata.dataFilePath{1}, [] };        
        cellPatternStats(end+1,:) = { 'FUCCI data', handles.data.metadata.dataFilePath{2}, [] };        
