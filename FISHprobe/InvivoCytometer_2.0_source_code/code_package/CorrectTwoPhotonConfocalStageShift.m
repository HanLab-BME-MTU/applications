function [mergedImageData, imRegValidMask, varargout] = CorrectTwoPhotonConfocalStageShift( twoPhotonImageData, twoPhotonPixelSpacing, confocalImageData, confocalPixelSpacing, varargin )
    ...
    if PARAMETERS.flagParallelize
        flagPoolOpenedAlready = isMatlabPoolOpen();        
    ...
    if ~isdeployed
        elastixParamFilePath = fullfile(fileparts( mfilename('fullpath') ), 'elastixParamFile_rigid3D.txt'); 
    else
        elastixParamFilePath = which('elastixParamFile_rigid3D.txt');        
    end
    elastixParamFilePath
    
    elastixParameters{1} = readElastixParameters(elastixParamFilePath);
