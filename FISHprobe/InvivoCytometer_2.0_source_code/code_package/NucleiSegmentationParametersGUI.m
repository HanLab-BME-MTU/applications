...
% Last Modified by GUIDE v2.5 08-Sep-2014 14:53:49
...
defaultParameters.thresholding.module = 'LocalSBRThresholdingModule';
defaultParameters.thresholding.parameters = LocalSBRThresholdingModule().parameters;
...
handles = updateUIControls( handles );
...
function [handles] = updateUIControls( handles )
    ...
    % thresholding
    handles.data.sampleData = []; 
    
	handles.data.thresholdingModuleList = {};
	handles.data.thresholdingModuleObjList = {};
	
	handles.data.thresholdingModuleList{end+1} = 'LocalSBRThresholdingModule';
	handles.data.thresholdingModuleObjList{end+1} = LocalSBRThresholdingModule('debugMode', true);
	
	handles.data.thresholdingModuleList{end+1} = 'LocalMinimumErrorThresholdingModule2D';
	handles.data.thresholdingModuleObjList{end+1} = LocalMinimumErrorThresholdingModule2D('debugMode', true);

	handles.data.thresholdingModuleList{end+1} = 'LocalOtsuThresholdingModule2D';
	handles.data.thresholdingModuleObjList{end+1} = LocalOtsuThresholdingModule2D('debugMode', true);

	handles.data.thresholdingModuleList{end+1} = 'LoGResponseThresholdingModule';
	handles.data.thresholdingModuleObjList{end+1} = LoGResponseThresholdingModule('debugMode', true);

	
    handles.data.thresholdingModuleNameList = cell( size(handles.data.thresholdingModuleList) );
    for i = 1:numel(handles.data.thresholdingModuleList)
        handles.data.thresholdingModuleNameList{i} = handles.data.thresholdingModuleObjList{i}.getName();
    end

    initModuleId = find( strcmp(handles.data.thresholdingModuleList, handles.data.curParameters.thresholding.module) );
    handles.data.thresholdingModuleObjList{initModuleId}.setParameters(handles.data.curParameters.thresholding.parameters);
    
    set( handles.popupThresholdingMethod, 'String', handles.data.thresholdingModuleNameList );    
    set( handles.popupThresholdingMethod, 'Value', initModuleId );    
...
% --- Executes on selection change in popupThresholdingMethod.
function popupThresholdingMethod_Callback(hObject, eventdata, handles)
% hObject    handle to popupThresholdingMethod (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns popupThresholdingMethod contents as cell array
%        contents{get(hObject,'Value')} returns selected item from popupThresholdingMethod

   curModuleId = get(handles.popupThresholdingMethod, 'Value');
   
   handles.data.curParameters.thresholding.parameters = handles.data.thresholdingModuleObjList{curModuleId}.parameters;
    
   % Update handles structure
   guidata(hObject, handles);

% --- Executes during object creation, after setting all properties.
function popupThresholdingMethod_CreateFcn(hObject, eventdata, handles)
% hObject    handle to popupThresholdingMethod (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in btnSetThresholdingParameters.
function btnSetThresholdingParameters_Callback(hObject, eventdata, handles)
% hObject    handle to btnSetThresholdingParameters (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

   curModuleId = get(handles.popupThresholdingMethod, 'Value');
   
   handles.data.thresholdingModuleObjList{curModuleId}.uiSetParameters();
...
% --- Executes on button press in btnTestThresholdingMethod.
function btnTestThresholdingMethod_Callback(hObject, eventdata, handles)
% hObject    handle to btnTestThresholdingMethod (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

    curModuleId = get(handles.popupThresholdingMethod, 'Value');
    curModuleObj = handles.data.thresholdingModuleObjList{curModuleId};
    
    imForegroundMask = curModuleObj.segmentImage(handles.data.sampleData.imageData, ...
                                                 handles.data.sampleData.metadata.pixelSize);
    
    imseriesmaskshow(handles.data.sampleData.imageData, ...
                     imForegroundMask, 'spacing', handles.data.sampleData.metadata.pixelSize);
                 
    set(gcf, 'Name', ['Result of ' curModuleObj.getName()]);
    
    % Update handles structure
    guidata(hObject, handles);

% --- Executes on button press in btnLoadSampleDataset.
function btnLoadSampleDataset_Callback(hObject, eventdata, handles)
% hObject    handle to btnLoadSampleDataset (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    ...
    imageData = matitk('FMEDIAN', ones(1,numel(metadata.pixelSize)), ...
                       double(mat2gray(imageData{metadata.channelIdNuclei}) * 4096) );
                        
    handles.data.sampleData.imageData = imageData;
    handles.data.sampleData.metadata = metadata;
    
    set(handles.btnTestThresholdingMethod, 'Enable', 'on');
