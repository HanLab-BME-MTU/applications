...
if nargin<5
    ...
    paramsIn.distCutOffForBody = 55;  % currently in um
...
for i = 1:length(frames)
    ...
    if isempty(distPath)
        ...
        if p.subRegions ==1;
            ...
            [subRois,xVect,yVect,h] =  GCAgetGrowthConeSubRegions20150306Rework(subRoiInfo,90,[ny,nx],4,mask);
            % save in a mask file for later use: Think about best place for
            % substructure to remain in line with movie data
            saveas(h,[saveDir filesep 'GCDefScatter.fig']);
            close gcf
