...
if nargin<5
    ...
    paramsIn.OutputDirectoryBodyShape = [saveDir filesep 'PARAMETER_EXTRACTION' filesep 'Descriptors' filesep 'Veil' filesep 'veil_size_measurements']; % for now I know this is
    % a bit weird to
    ...
    % To turn on
    paramsIn.GCFinder.on = false;
    paramsIn.GCFinder.neckCutOff = 1; % flag to Find GCS
    paramsIn.GCFinder.partitionGC.on = true;
    paramsIn.GCFinder.partitionGC.angle = 45;
    paramsIn.GCFinder.troubleShootMasks = true; % to make the plots
...
if p.GCFinder.on == true ;
    
    subRegDir = [p.OutputDirectory filesep 'Output' filesep 'GCMasks'];
    ...
    % make the directories for the sanity plot if desired
    if p.GCFinder.troubleShootMasks == true
        plotFrontMaskDir1 = [p.OutputDirectory filesep 'Output' filesep 'GCMasks' filesep 'Troubleshoot'];
        mkClrDir(plotFrontMaskDir1);
    end
    if p.GCFinder.partitionGC.on == true; % if further partition GC
        % Make the GC Mask Directories
        subRegDir2 = [p.OutputDirectory filesep 'Output' filesep 'GCMasksRegional'];
        mkClrDir(subRegDir2); % need to make a directory to save the output masks
        subNamesReg{1} = 'FrontGC';
        subNamesReg{2} = 'BackGC';
        subNamesReg{3} = 'SideGC';
        
        % make the mask file directories
        arrayfun(@(i) mkClrDir([subRegDir2 filesep subNamesReg{i} filesep 'masks']),1:3);
        
        % make the corresponding sanity check directory if desired.
        if p.GCFinder.troubleShootMasks == true
            plotFrontMaskDir2 = [p.OutputDirectory filesep 'Output' filesep 'GCMasksRegional' filesep 'Troubleshoot'];
            mkClrDir(plotFrontMaskDir2);
        end % if troubleshootmasks
        
    end % if partionGC
end % if p.GCFinder.on == true
%% START
error = 0;
...
for i = 1:length(frames)
    ...
    if isempty(distPath)
        ...
        % Get some information for the subRoi making and save in case the
        % user decides to run later.
        distTrans = bwdist(~mask);
        distVals = distTrans(pixForBodyEst);
        subRoiInfo = [double(pixForBodyEst) double(distVals)];
        subRoiInfoAllFrames{iFrame} = subRoiInfo;
        save([p.OutputDirectory filesep 'subRoiInfo.mat'],'subRoiInfoAllFrames'); % always save the subRoi info in case the user chooses to call
        % this later
        %%%%%%%END GET NEURITE LENGTHS   (Should make into smaller functions)
        
        %% MAKE SUBREGIONS: If user specifies
        if p.GCFinder.on == true
            
            % Find the subRois corresponding to the growth cone and the
            % stem
            mask = logical(mask); 
            [subRois,xVect,yVect,subRoiInfoGC,h] =  GCAgetGrowthConeSubRegions20150306Rework(subRoiInfo,mask,'GCFinder',p.GCFinder);
            
            % Save the distance transform plots along longest path
            if p.GCFinder.troubleShootMasks == true;
                if ~isdir([subRegDir filesep 'DistTransProfile']);
                    mkdir([subRegDir filesep 'DistTransProfile']);
                end
                
                saveas(h,[subRegDir filesep 'DistTransProfile' filesep num2str(iFrame,'%03d') '.fig']);
                saveas(h,[subRegDir filesep 'DistTransProfile' filesep num2str(iFrame,'%03d') '.tif']);
                
            end % if p.GCFinder.on.troubleshootPlots
            close gcf
            ...
            if p.GCFinder.partitionGC.on == true;
                GCMask = subRois(:,:,1);
                [subRoisReg, xVectR,yVectR] = GCAgetGrowthConeSubRegions20150306Rework(subRoiInfoGC,GCMask);
                % Write the masks to the appropriate directory.
                arrayfun(@(i) imwrite(subRoisReg(:,:,i),[subRegDir2 filesep subNamesReg{i} filesep 'masks' filesep 'mask' num2str(iFrame,'%03d') '.tif']),1:3);
            end
    ...
    if ~isempty(p.distCutOffForBody);
        ...
        if ( p.GCFinder.on == true && p.GCFinder.troubleShootMasks == true)
            ...
            saveas(gcf,[plotFrontMaskDir1 filesep  num2str(iFrame,'%03d') '.fig']);
            saveas(gcf,[plotFrontMaskDir1 filesep  num2str(iFrame,'%03d') '.png']);
            close gcf
            
        end
        
        
        if ( p.GCFinder.on == true && p.GCFinder.troubleShootMasks == true && p.GCFinder.partitionGC.on == true)
            c = ['r','g','b'];  % for now front is red, green is back, and blue is sides.
            ...
            for iSub = 1:3
                ...
                roiYXSub = bwboundaries(subRoisReg(:,:,iSub));
                
                cellfun(@(x) plot(x(:,2),x(:,1),'color',c(iSub),'Linewidth',3),roiYXSub);
            end % iSub
            [yFront,xFront] = ind2sub([ny,nx],subRoiInfoGC(:,1));
            ...
            u = xVectR(1) - xVectR(end); % vector towards the tip of neurite
            v = yVectR(1) - yVectR(end);
            
            % Plot the direction of the local vector in black
            
            quiver(xVectR(end),yVectR(end),u,v,10,'color','k','Linewidth',2);
            
            plot(xFront,yFront,'color','k','linewidth',1);
            % scatter the points used in the calculation of local vector.
            scatter(xVectR(1),yVectR(1),50,'k','fill');
            scatter(xVectR(end),yVectR(end),50,'k','fill');
            % scatter the center point around which the local vector was
            % plotted
            %scatter(xCenter,yCenter,100,'w','fill','p');
            ...
            saveas(gcf,[plotFrontMaskDir2 filesep  num2str(iFrame,'%03d') '.fig']);
            saveas(gcf,[plotFrontMaskDir2 filesep  num2str(iFrame,'%03d') '.png']);
            close gcf
...
if ~isempty(p.distCutOffForBody)
    paramC = bodyShape;
    save([p.OutputDirectoryBodyShape filesep 'bodyShape.mat'],'paramC','timeStamp');
