function [ h ] = gcaPlotNeuriteLengthInTime( neuriteIn ,varargin)
%Overview
%%
%%Input check
ip = inputParser;
...
ip.addParamValue('timeInterval',5,@isscalar);
ip.addParamValue('makeMovie',1,@isscalar);
ip.addParamValue('saveDir',[],@ischar);
...
smoothedOnly = ip.Results.smoothedOnly;
dataType = ip.Results.dataType;
makeMovie = ip.Results.makeMovie;
timeInterval = ip.Results.timeInterval;
...
if strcmpi(ip.Results.plotType,'subplot')
    fSizeLabels = 20;
    fSizeAxis = 10;
else
    fSizeLabels = 20;
...
[outgrowthFilt , outlierIdx] = gcaFindOutliersFromMedFilt(neuriteIn,10,3,false);
...
if strcmpi(dataType,'vel');
    ...
    if ~isempty(saveDir)
        ...
        saveas(gcf,[saveDir filesep 'Outgrowth_Velocity' '.fig']);
    end
end
if makeMovie== 1
