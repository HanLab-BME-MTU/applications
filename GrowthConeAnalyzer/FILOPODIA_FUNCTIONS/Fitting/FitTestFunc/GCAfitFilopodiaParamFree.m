...
ip.addParameter('OutputDirectory2',pwd,@ischar); 
...
%% Quick and Dirty : Estimate the number of sigmoidals                                
                ...
                % for now just make quick and dirty file for visualization.  
                
                [slopeMaxNeg,slopeMaxPos] = gcaFindPotentialSigmoidals(yData,'makePlot',false); 
...
%                % Deal with the External and Internal filopodia fits
%                % differently ... Internal 
%                switch toAdd 
%                    case 'Ext'
               
               
               
               %% find the sigmoidal sites that lie above the background estimate for a quick and dirty
                 
                % for now estimate the background from the local filo
                % environment- pixels larger than (might need a larger sample to get the variance?) 
                
                % quick fix : think there is a small bug for the mask indices for at the edge.. 20151124 need to check 
                % 
%              
                    indicesBack = maskIndices([1:2,4:5],end-11:end); % reformated above so that 3 is the primary axis 
...
%                 end 
                ...
                backEst = imgFilt(indicesBack(:)) ; % NOTE make a parameter value 
                backEstMean = nanmean(backEst);  
   
            
                % perform fitting around the first sigmoidal closest to the noise 
                ...
                % check to make sure not including increasing portions
                % (should check if signficiant); 
                if ~isempty(slopeMaxPos) % you have increasing portions
                    % make sure that do not have an increasing portion
                    % between tentative sigmoidal mean and end point
                    if (sum(endFit>slopeMaxPos)>0 && sum(slopeMaxPos > slopeMaxNeg(end))>0);
                       idx =  find(endFit>slopeMaxPos);
                        endFit = slopeMaxPos(idx(1)) -2; 
                    end
                    % make sure that do not have an increasing portion
                    % between tentative sigmoidal mean 
...
%                 if delt1<10 
%                     yDataFit = yData(slopeMaxNeg
%                 yDataFit = yData(slopeMaxNeg-10:slopeMaxNeg+10); 
                ...
                if      (~isempty(yData) && length(yData) > 4) % make a cut off that length yData needs to be at least four pixels..
                    ...
                    if p.TSOverlays ==1
                        ...
                        hold on 
                        % mark the potential sigmoidal sites 
                        scatter(distFilo(slopeMaxNeg),yData(slopeMaxNeg),50,'r','filled'); 
                        scatter(distFilo(slopeMaxPos),yData(slopeMaxPos),50,'g','filled');
                        % mark the potential background thresh
                        line([0 distFilo(end)], [backEstMean,backEstMean],'color','k','Linestyle','--');
