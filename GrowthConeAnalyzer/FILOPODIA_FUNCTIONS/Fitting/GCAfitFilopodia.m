...
for iType = typeStart:typeEnd
    ...
    for ifilo = 1:numFilo2Fit
        ...
        % sometimes particularly in int (embedded) filo structure have []
        ...
        if isempty(intersect(filoInfo(idxCurrent).Ext_pixIndicesFor,filoInfo(idxCurrent).Ext_pixIndicesBack));
            ...
            if ~isempty(xyFilo);
                ...
                % helpful for calculating branch distances..
                filoInfo(idxCurrent).([toAdd 'distFilo']) = distFilo;
                
                if strcmpi(p.ValuesForFit,'Intensity') % might remove this option entirely
                    maskIndices = filoInfo(idxCurrent).([toAdd 'maskIndices']);
                    
                    %  make sure to truncate internal maskIndices that span
                    %  veil/stem mask
                    ...
                    % These were saved originally with the first column the center line and
                    % the perpendicular pixels outward...
                ...
                yData = yData(~isnan(yData)); % sometimes these data are padded with NaNs
