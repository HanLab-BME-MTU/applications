...
% GCAgetNeuriteOrient: (STEP I Veil/Stem Seg of GCA package) 
%
...
%
% Backbone seed candidates are obtained via a 4th order steerable 
% filtering process followed by non-maximal suppression, followed by some 
% post-processing steps
% Post Processing 
% I. Junction break, 
% II. A small size filter (potentialy to be made optional or to include all) and
% a linking step to link co-linear ridges within a certain distance.
% III. Small co-linear ridge linking step- need to make sure can handle singletons and has a directionality. 

% All ridge candidates with an endpoint within 10 pixels of the image 
% boundary are considered backbone seed candidates, if no candidates are 
% found this search radius is widened to 20 pixels 
%
% A small 'connect linear pieces' step helps link small spatial gaps
% between colinear pieces before the candidate backbone seeds are 
...
% upon this initial backbone seed to collect high confidence pieces of the 
% body in later steps. 


% INPUT: 
%   img: a NxM array of the image where N and M correspond to the image size 
%   p: a structure of input parameters including fields 
%       
% OUTPUT 
...
% Estimate the background of the image based on intensity
        [maskBack,~,~] = estimateBackgroundArea(img);
        
        % Calculate LargeScale Ridges 
        [maxResLarge, maxThLarge ,maxNMSLarge ,scaleMapLarge]= multiscaleSteerableDetector(img,4,p.BBScale);
        ...
        % mask scales by NMS (the maximum in the direction of the ridge) 
%         maskedByNMS_Scales = (maxNMSLarge>0).*scaleMapLarge;
%         
%         largeScaleMask = (maskedByNMS_Scales>1.5).*maskedByNMS_Scales; 
%         
%         maxNMSLarge = largeScaleMask.*maxNMSLarge; 
        ...
        cutoff = prctile(values,25); % note arbitrarily set this cut-off to the 25th percentile- tried to fit the first mode of 
...
%         
      %%  Finish Optional TroubleShoot Plot I: RidgeCand Before After Clean
        ...
        % (arbitrary assignment)
        csizeRidgeFirst = cellfun(@(x) length(x),CCRidgeBone.PixelIdxList);
        CCRidgeBone.PixelIdxList(csizeRidgeFirst < 5) = [];
        CCRidgeBone.NumObjects = CCRidgeBone.NumObjects - sum(csizeRidgeFirst<5);
        cleanedRidgeLabels = labelmatrix(CCRidgeBone);
        cleanedRidge = cleanedRidgeLabels>0;
        ...
        if isempty(idxEnterNeurite) % need to interpolate to make sure closed contour
            ...
            [idxBoundaryClose,dist] = KDTreeBallQuery([xBoundary,yBoundary] ,EPsBackbone,10);
            % quick fix
            ...
            if isempty(distAll)
                ...
                [idxBoundaryClose,dist] = KDTreeBallQuery([xBoundary,yBoundary],[xBBAll,yBBAll],10);
                numPixBBTested = length(xBBAll);
                distAll =vertcat(dist{:});
                if isempty(distAll)
                    [idxBoundaryClose,dist] = KDTreeBallQuery([xBoundary,yBoundary],EPsBackbone,20); % try a larger search radius
                    distAll = vertcat(dist{:});
                    numPixBBTested = length(EPsBackbone(:,1));
                end
            end
            if ~isempty(distAll);
                ...
                linkCoords = bresenham([BBLinkers(:,1),BBLinkers(:,2)],[xBoundary(E(toSave,2)),yBoundary(E(toSave,2))]);
