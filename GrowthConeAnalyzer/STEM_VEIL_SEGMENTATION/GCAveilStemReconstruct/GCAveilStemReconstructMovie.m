...
if nargin < 2
    ...
    if exist(patchSizeFilename,'file')==2
        ...
        paramsIn.patchSize = 75; % Default
    end
    
    paramsIn.restart.startFrame = 'auto';   % 'auto'
    paramsIn.restart.endFrame = 'auto';
    
    % need to make an option for reloading the params ...
...
for iCh = 1:nChan
    ...
    for iFrame = startFrame:endFrame
        ...
        if paramsIn.makeMovie == 1;
            ...
            hold on
            saveDirMov =  [paramsIn.OutputDirectory filesep 'SmallMovie' filesep 'Frame' num2str(iFrame,'%03d') ];
            if ~isdir(saveDirMov)
                mkdir(saveDirMov)
            end
            pixSizeMic = 0.216;
    ...
    if p.plots ==1
        ...
        cd([outDirChan filesep 'troubleShootVeilStem'])
        execute = 'mencoder mf://*.png -mf w=800:h=600:fps=5:type=png -ovc lavc -lavcopts vcodec=mpeg4:mbd=2:trell -oac copy -o movie.wmv';
        system(execute);
    end
