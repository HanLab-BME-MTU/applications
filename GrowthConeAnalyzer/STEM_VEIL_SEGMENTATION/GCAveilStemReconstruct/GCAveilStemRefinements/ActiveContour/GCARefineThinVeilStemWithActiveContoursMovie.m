function [ output_args ] = GCARefineThinVeilStemWithActiveContoursMovie(movieData,varargin)
...
%% 
for iFrame = 75:nFrames   
    veilStemMaskC = veilStem(iFrame).finalMask;    
    img = double(imread([movieData.getChannelPaths{channels(iCh)} filesep movieData.getImageFileNames{channels(iCh)}{iFrame} ])); 
    [veilStemMaskRefineC, TSFigs,maxNMSInt,scaleMapInt] =  GCARefineThinVeilStemWithActiveContours(img,veilStemMaskC,p);    
    veilStem(iFrame).finalMask = veilStemMaskRefineC;  
    veilStem(iFrame).maxNMSInt  = maxNMSInt; 
    veilStem(iFrame).scaleMapInt = scaleMapInt; 
    
    save([outDirC filesep 'veilStemR_AC.mat'],'veilStem') ;    
end 
