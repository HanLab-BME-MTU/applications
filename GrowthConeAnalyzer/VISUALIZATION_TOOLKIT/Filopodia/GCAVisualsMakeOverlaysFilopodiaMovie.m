function [ output ] = GCAVisualsMakeOverlaysFilopodiaMovie(movieData,paramsIn)
    ...
    if strcmpi(paramsIn.colorScheme,'multi');
        ...
        for ifilo = 1:length(filoInfoC)
            ...
            if filterSetIdx == 1;
                ...
                GCAVisualsMakeOverlaysFilopodiaWithFiltInput(filoInfoIdx,[ny,nx],filterSetIdx,1,c(ifilo,:),valuesIdx,1);
                clear filoInfoIdx filterSetIdx valuesIdx
            end % if filterSetIdx
        end % if ifilo
    else
        GCAVisualsMakeOverlaysFilopodiaWithFiltInput(filoInfoC,[ny,nx],filterSetC,1,[],valuesC); % plot in red
    end
    hold on
