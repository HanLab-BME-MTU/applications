function [POS,i1,i2,tmtr,cx,cy]=PlotTrackingResultsY(handles,st)
    ...
    PeakInd=handles.sPeakInd{1};
    Rad=handles.Rad{1};
    PeakInd=PeakInd(Rad(PeakInd)>fw);
    if fl<fw
        [~,~,cx{1},cy{1}]=FindConesX(PeakInd,Rad,handles.Coor{1},handles.PathInd{1},handles.PathLng{1},fl);
    end
    ...
    for n=2:mp 
        ...
        PeakInd=handles.sPeakInd{n};
        Rad=handles.Rad{n};
        PeakInd=PeakInd(Rad(PeakInd)>fw);
        if fl<fw
            [~,~,cx{n},cy{n}]=FindConesX(PeakInd,Rad,handles.Coor{n},handles.PathInd{n},handles.PathLng{n},fl);
        end
