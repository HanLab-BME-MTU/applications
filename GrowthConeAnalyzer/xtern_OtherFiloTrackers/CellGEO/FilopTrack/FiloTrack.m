function varargout = FiloTrack(varargin)
% FILOTRACK MATLAB code for FiloTrack.fig
%      FILOTRACK, by itself, creates a new FILOTRACK or raises the existing
%      singleton*.
%
%      H = FILOTRACK returns the handle to a new FILOTRACK or the handle to
%      the existing singleton*.
%
%      FILOTRACK('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FILOTRACK.M with the given input arguments.
%
%      FILOTRACK('Property','Value',...) creates a new FILOTRACK or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FiloTrack_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FiloTrack_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FiloTrack

% Last Modified by GUIDE v2.5 03-Jun-2013 18:06:21

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @FiloTrack_OpeningFcn, ...
                   'gui_OutputFcn',  @FiloTrack_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before FiloTrack is made visible.
function FiloTrack_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FiloTrack (see VARARGIN)

% Choose default command line output for FiloTrack
    handles.output = hObject;
    def=importdata('Defaults.dat');
    
    handles.Time = 1;
    handles.DefWidth = def(1)/2;
    handles.DefLength = def(2);
...
function varargout = FiloTrack_OutputFcn(hObject, eventdata, handles)
...
function figure1_DeleteFcn(hObject, eventdata, handles)
    try 
        def = [2*handles.Width, handles.Length, handles.Distance, handles.Gap, handles.Filter];
    ...
    if fr~=oldfr
        ...
        if ~istracking    
            ...
            if handles.ShPh
                ...
                set(handles.text5,'String',['Mean Length = ' num2str(mL,'%.2f')]);
                ...
                if handles.shownreslt
                    ...
                    ylabel(handles.top_axis,'Filopodia Number');%,'FontName','TimesNewRoman');
                    title(handles.top_axis,['<N> = ' num2str(mean(handles.mN),'%.2f') ' ' char(177) ' ' num2str(std(handles.mN),'%.2f') ...
                           ...
                           '  ( <L> - L_{cr} = ' num2str(mean(handles.mL(handles.mL>0)-fl),'%.2f') ' )']);
                    ...
                    ylabel(handles.bottom_axis,'Filopodia Length');%,'FontName','TimesNewRoman');
            ...
            if handles.showntrack
                ...
                FiloDist(handles,fr,fw,fl,fd,fp);
            else
...
function mapper2(handles,src,evnt) 
    ...
    set(handles.text2,'String',['critical width = ' num2str(2*fw,'%.1f')]);    
...
function mapper3(handles,src,evnt) 
    ...
    set(handles.text3,'String',['critical length = ' num2str(fl,'%.1f')]);
...
function mapper7(handles,src,evnt) 
    ...
    hold on;
    plot([pXb; pXb(1)],[pYb; pYb(1)],'Color','b');
    FiloDist(handles,fr,fw,fl,fd,pr);
    axis ij;
    axis image;
    axis off;
    set(handles.left_axis,'XLim',XLim,'YLim',YLim);
...
function import(hObject)
    ...
    if filterindex~=0
        ...
        if mp>1   
            ...
            if handles.DefWidth > 10
                handles.Width = 10;
            else
                handles.Width = handles.DefWidth;
            end
            if handles.DefLength > 20
                handles.Length = 20;
            else
                handles.Length = handles.DefLength;
            end
                        
            if handles.DefDistance > 20
                handles.Distance = 20;
            else
            ...
            if handles.ShTr
                ...
                set(handles.text1,'String',['time = ' num2str(fr)]);
                set(handles.text2,'String',['critical width = ' num2str(2*fw,'%.1f')],'ForegroundColor','k');
                set(handles.text3,'String',['critical length = ' num2str(fl,'%.1f')],'ForegroundColor','k');
                set(handles.text4,'String','disabled','ForegroundColor',[.5 .5 .5]);
                set(handles.text5,'String','disabled','ForegroundColor',[.5 .5 .5]);
                set(handles.text5a,'String','disabled','ForegroundColor',[.5 .5 .5]);
                ...
                if handles.ShPh
                    ...
                    tuck=zeros(1,Lt);
                    for i=1:Lt 
                        [~,filo{i},pd(i)]=FindPathCond(sINDX{fr},sCoor{fr},sDIST{fr},sRad{fr},sVb{fr}(sTip{fr}(i)),fw,handles.CutOff);                        
                        if filo{i}(end,1)>0
                            tuck(i)=filo{i}(end,1);
                        else
                            tuck(i)=filo{i}(end-1,1);
                        end                        
                    end    
                    %}
                    Lt=length(sTip{fr});
                    ...
                    set(handles.text5,'String',['Mean Length = ' num2str(mL,'%.2f')],'ForegroundColor','k');
...
try
    ...
    if handles.showntrack
        ...
        %[handles.POS,handles.LEN,handles.i1,handles.i2,handles.tmtr]=PlotTrackingResults(handles,1);
...
try
    ...
    crW=2*fw; crL=fl; crD=fd; crG=fg; crF=ff;
