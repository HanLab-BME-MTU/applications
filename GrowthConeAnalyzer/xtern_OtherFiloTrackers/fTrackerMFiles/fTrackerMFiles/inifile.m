...
%        file. It creates, reads, or writes data from/to a standard ini
%        file. Such a file is organized into sections
%       ([section name]), subsections(enclosed by {subsection name}),
%       and keys (key=value).  Empty lines and lines with first non-empty
...
%                           If not given and the key is not found, an
%                           empty value is returned.
%                           It MUST be in the format as given by
%                           value, e.g. if value = 'i' it must be
%                           given as integer etc.
%
%
%   EXAMPLE:
...
%                'data','','x',x};
%   inifile('test1.ini','writest',writeKeys,'plain');
%
...
elseif (strcmpi(operation,'read'))
    ...
    for ii=1:m  % go through all the keys
