% Francois Aguet, October 2010 (last modified: 05/30/2012)
...
parfor i = 1:length(data)
    main(data(i), alpha, overwrite);
end
    

function main(data, alpha, overwrite)

kLevel = norminv(1-alpha/2.0, 0, 1); % ~2 std above background
% kLevel = norminv(1-alpha, 0, 1); % ~1.64 std above background

% load tracks (all)
% fprintf('Loading tracks for %s ...\n', getShortPath(data))
ts = load([data.source 'Tracking' filesep 'ProcessedTracks.mat']);
...
if ~isfield(ts.tracks, 'significantSignal') || overwrite
    ...
    nPos = sum([ts.tracks.significantSignal],2);
    for c = sCh
    ...
    for k = 1:nt
        ...
        if ts.tracks(k).catIdx==1 && numel(ts.tracks(k).t)>4
    ...
    save([data.source 'Tracking' filesep 'ProcessedTracks.mat'], '-struct', 'ts');
else
