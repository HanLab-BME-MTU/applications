...
mat_x     = full(lftInfo.Mat_xcoord);
mat_y     = full(lftInfo.Mat_ycoord);
mat_da    = full(lftInfo.Mat_disapp);
...
for i=1:length(gtpos)
    ...
    if ~isempty(pos_a)        
        ...
        mpm_t1(k,startpos:pos_a-1) = startpos-pos_a:-1;
        % fill in trajectory times
        mpm_t1(k,pos_a:endpos) = 0:endpos-pos_a; 
    end
    ...
    if ~isempty(pos_da)
        ...
        mpm_t2(k,pos_da+1:endpos) = 1:endpos-pos_da;
        % fill in trajectory times
        mpm_t2(k,startpos:pos_da) = startpos-pos_da:0;
    end      
end

MPMallpos = NaN(nx,2*nf,3);
