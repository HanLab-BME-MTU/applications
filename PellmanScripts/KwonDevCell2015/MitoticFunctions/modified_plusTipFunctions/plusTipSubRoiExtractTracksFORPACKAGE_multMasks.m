function [projData,M]=plusTipSubRoiExtractTracksFORPACKAGE_multMasks(subRoiDir,varargin)
...
% wholeCellRoiMask is the roi_x mask (woops change so don't save everything
% in wrapper rosin...
%roiYXWhole = load([upTwo filesep 'roiYXSeg.mat']); 

%wholeCellRoiMask = roiYXWhole.maskSeg; 
%wholeCellRoiMask=logical(imread([upTwo filesep 'roiMaskSeg.tif']));

%% Change such that can read mult masks
% load masks 
if multMasks == 1
...
masksOtherSide = searchFiles('.tif',[], masksOtherSideDir ,0); 


%roiMaskOtherSide = logical(imread([masksOtherSide{1,2} filesep masksOtherSide{1,1}])); 
...
%     if multMasks ==1
%         nTotMasks = length(masks); 
%         for iFrame = 1:nTotMasks
%         % filter only those for that end in a given frame 
%         roiMask = double(imread([masks{iFrame,2} filesep masks{iFrame,1}]));
%         % find all subTracks in that frame 
%       
%         %get original idx
%         idxOrigFull = find(lastPtFrIdx ==iFrame); 
%         
%         % truncate xMat to that frame 
%         xMat_iFrame = xMat(lastPtFrIdx==iFrame,:);
%         yMat_iFrame = yMat(lastPtFrIdx==iFrame,:); 
%        
%         if ~isempty(idxOrigFull)
%             
%             
%         % convert to pixelIdx 
%         pixIdxLastPts_iFrame = sub2ind(size(roiMask),ceil(yMat_iFrame(:,iFrame)-.5),ceil(xMat_iFrame(:,iFrame)-.5));
%        
%           firstPtFrIdx_iFrame=arrayfun(@(i) find(~isnan(xMat_iFrame(i,:)),1,'first'),1:length(xMat_iFrame(:,1)))';
%         
%         pixIdxFirstPts_iFrame = sub2ind(size(roiMask),ceil(yMat_iFrame(:,firstPtFrIdx_iFrame)-.5),ceil(xMat_iFrame(:,firstPtFrIdx_iFrame)-.5)); 
%         
%         endInRegion= logical(roiMask(pixIdxLastPts_iFrame));
%         startInRegion = logical(roiMask(pixIdxFirstPts_iFrame)); 
%         boundCrossIn = (endInRegion & ~startInRegion);
%         % get the trckIDs
%         
%         % get the xMats
%         xMatInMultMask{iFrame} = xMat_iFrame(boundCrossIn,:);
%         yMatInMultMask{iFrame} = yMat_iFrame(boundCrossIn,:); 
%         trckIdxInRegion{iFrame} = idxOrigFull(boundCrossIn,:) ; 
%         else 
%         xMatInMultMask{iFrame} = []; 
%         yMatInMultMask{iFrame} = []; 
%         trckIdxInRegion{iFrame} = []; 
%         end % if isempty
%         end % iFrame
% 
%     xMatInMultMaskFinal = vertcat(xMatInMultMask{:});
%     yMatInMultMaskFinal = vertcat(yMatInMultMask{:});
%     trckIdxInRegion{iFrame} = vertcat(trckIdxInRegion{:}); 
%     else % proceed with below
    ...
    if multMasks == 1 ;
        ...
        for iFrame = 1:length(masks)-1 
            display(['extracting' num2str(iFrame)])
           ...
           % masksOrig = searchFiles('.tif',[],[up3 filesep 'masks'],0);  
            ...
            %  pixInd_iFrame = pixInd(idx_subTrackToKeep);
            %              INMult{iFrame} = roiMask(pixInd_iFrame);
            ...
            % plot sanity checks 
             if ~isempty(xMat_iFrame) 
               ...
               if ~isdir([subRoiDir filesep 'multMaskTroubleshoot']); 
                mkdir([subRoiDir filesep 'multMaskTroubleshoot']); 
               end 
                saveas(gcf,[subRoiDir filesep 'extract_plots' filesep 'multMaskExtract' filesep num2str(iFrame,' %02d') '.tif']); 
                saveas(gcf,[subRoiDir filesep 'multMaskTroubleshoot' filesep  num2str(iFrame,'%03d') '_a' '.tif']); 
    ...
    %% if do a cortical calculation is on make small section to potentially extract the distance transform of each these points as check
    testDistTransPop =1 ;    
    %find which tracks have their last point within the subregion of interest
    % idx based on dataMatMerge will be a one if in region zero if not 
    % want to find those with a 1 
    inIncludeRegionLast=find(roiMask(pixIdxLast));
    inIncludeRegionFirst=find(roiMask(pixIdxFirst));
   
     if testDistTransPop == 1
        % perform dist transform (from whole cell mask as we are most
        % interested in the distance 
        distTrans = bwdist(~wholeCellRoiMask);
        % mask of microtubule endpoints
        endPtMask = zeros(size(distTrans));
        endPtMask(pixIdxLast) = 1; 
        % combine masks so just get distTrans of endpoint coords
        distValues = endPtMask.*roiMask.*distTrans;
         testPlot =1; 
         if testPlot ==1; 
         % plot 
         ...
         hold on 
         toPlot= bwboundaries(wholeCellRoiMask); 
         plot(toPlot{1}(:,2),toPlot{1}(:,1),'k'); 
         title('Distance Measurements In Pixels of All Microtubule End Point Coords In Region'); 
         colorbar
         saveas(gcf,[subRoiDir filesep 'extract_plots' filesep 'DistPlot.eps'],'psc2');
         
         close(gcf)
         end 
        
        
        distValues = distValues(distValues~=0); 
        projData.subRegionDistValues = distValues; 
      
       
        
        % test for comets crossing out of region 
        endPtMask = zeros(size(distTrans));
       
        endPtMask(pixIdxLast) = 1;
        % find those pixels that lie outside the segmentation mask: note obviously will work if used a wider mask 
        % to start with for detect/tracking which is typically my practice in the end you could dilate the seg mask to check for 
        % detection outside of region...
        maskOut = endPtMask.*~wholeCellRoiMask; 
        CCOut = bwconncomp(maskOut); 
        numOut = CCOut.NumObjects; 
        % get perimeter of border 
        props =  regionprops(wholeCellRoiMask,'Perimeter') ;
        perim = props.Perimeter; 
        projData.NumCometsOutOfMaskPerPix = numOut/perim; % parameter signifies a bad segmentation mask 
...
%% if refine the cortical dwell times... 
%  if (multMasks ==1 && inOnly ~=1) % if inonly don't bother with dwell because meaningless!! 
% refineDwell = 1;% need to fix so not reliant on the bound cross in parameter
%  else 
     refineDwell =1;
%  end
if refineDwell == 1;
    ...
    for iSubTrack = 1: length(xMatIn(:,1)) % just set entire thing up as a loop for now...
        ...
        endPtDilate =  imdilate(endPtMask,strel('disk',2)); % create a mask dil
        endPtDilate(1,1) = 0;
...
%    figure;
%  hold on 
%  numDwell = length(xCoordsDwell(:,1)); 
%  cmap = colormap(lines(numDwell)); 
%   
%  for i = 1:numDwell; 
%      scatter(xCoordsDwellOut(i,:),yCoordsDwellOut(i,:),'w','filled')
%      scatter(xCoordsDwell(i,:),yCoordsDwell(i,:),[],cmap(i,:),'filled');
%      plot(xCoordsDwellOut(i,:),yCoordsDwellOut(i,:),'color','w'); 
%      plot(xCoordsDwell(i,:),yCoordsDwell(i,:),'color', cmap(i,:)); 
%      
%     toPlot =  bwboundaries(dwellMasks(:,:, i));
%     cellfun(@(x) plot(x(:,2),x(:,1),'color',cmap(i,:)),toPlot);
%     
%     clear toPlot
%     
...
% this is not that classy of a way to output the data but it will do 
if refineDwell == 1
