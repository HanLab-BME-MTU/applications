...
force_x7 = assumedForceShifted(1,x_mat_u,y_mat_u,fpos(7,1),fpos(7,2),fvec(7,1),fvec(7,2),forceType,'largeFA');
force_y7 = assumedForceShifted(2,x_mat_u,y_mat_u,fpos(7,1),fpos(7,2),fvec(7,1),fvec(7,2),forceType,'largeFA'); %group2, large one
...
force_y14 = assumedForceShifted(2,x_mat_u,y_mat_u,fpos(14,1),fpos(14,2),fvec(14,1),fvec(14,2),forceType,'largeFA'); %group2, large one
...
[ux, uy]=fwdSolution(x_mat_u,y_mat_u,E,[],[],[],[],...
    ...
    assumedForceShifted(1,x,y,28*imScale,27*imScale,850,-280,forceType,'largeFA')+...
    ...
    assumedForceShifted(1,x,y,40*imScale,33*imScale,700,-1000,forceType,'largeFA')+...
    ...
    assumedForceShifted(2,x,y,28*imScale,27*imScale,850,-280,forceType,'largeFA')+...
    ...
    assumedForceShifted(2,x,y,40*imScale,33*imScale,700,-1000,forceType,'largeFA')+...
...
[fx_FastBEMreg2,fy_FastBEMreg2,~,~,~,~,~,~,sol_mats_reg2]...
    = BEM_force_reconstruction(x_mat_f,y_mat_f,ux_f,uy_f,forceMeshFastBEM,...
    E,L,[],[],'fast',meshPtsFwdSol,...
    'LaplacianReg','basisClassTblPath',basisClassTablePath,...
...
save([dataPath '/MandforceMesh.mat'],'M_FastBEM','M_FastBEMfine','forceMeshFastBEM','forceMeshFastBEMfine',...
    'fx_FastBEM1nfine','fy_FastBEM1nfine','fx_FastBEMreg2fine','fy_FastBEMreg2fine','fx_FastBEMregfine',...
    'fy_FastBEMregfine','force_FTTCfine','-v7.3');
...
% For force 1
tnorm1D_org_g1 = fnorm_org(fpos(14,2),fpos(14,1)-5*imScale:fpos(14,1)+5*imScale);
tnorm1D_FTTCreg_g1 = fnorm_fttcreg(round(fpos(14,2)/5)+1,round(fpos(14,1)/5)+1-round(5*csScale):round(fpos(14,1)/5)+1+round(5*csScale));
tnorm1D_FastBEMreg_g1 = fnorm_FastBEMreg(round(fpos(14,2)/5)+1,round(fpos(14,1)/5)+1-round(5*csScale):round(fpos(14,1)/5)+1+round(5*csScale));
tnorm1D_FastBEMreg2_g1 = fnorm_FastBEMreg2(round(fpos(14,2)/5)+1,round(fpos(14,1)/5)+1-round(5*csScale):round(fpos(14,1)/5)+1+round(5*csScale));
tnorm1D_FTTCregfine_g1 = fnorm_fttcregfine(fpos(14,2),fpos(14,1)-5*imScale:fpos(14,1)+5*imScale);
tnorm1D_FastBEMregfine_g1 = fnorm_FastBEMregfine(fpos(14,2),fpos(14,1)-5*imScale:fpos(14,1)+5*imScale);
tnorm1D_FastBEMreg2fine_g1 = fnorm_FastBEMreg2fine(fpos(14,2),fpos(14,1)-5*imScale:fpos(14,1)+5*imScale);
tnorm1D_FastBEMn1fine_g1 = fnorm_FastBEMn1fine(fpos(14,2),fpos(14,1)-5*imScale:fpos(14,1)+5*imScale);
x1D_g1 = grid_mat_f(round(fpos(14,2)/5)+1,round(fpos(14,1)/5)+1-round(5*csScale):round(fpos(14,1)/5)+1+round(5*csScale),1);
x1Du_g1 = grid_mat_u(fpos(14,2),fpos(14,1)-5*imScale:fpos(14,1)+5*imScale,1);
...
tnorm1D_org_g2 = fnorm_org(fpos(7,2),fpos(7,1)-5*imScale:fpos(7,1)+5*imScale);
tnorm1D_FTTCreg_g2 = fnorm_fttcreg(round(fpos(7,2)/5)+1,round(fpos(7,1)/5)+1-round(5*csScale):round(fpos(7,1)/5)+1+round(5*csScale));
tnorm1D_FastBEMreg_g2 = fnorm_FastBEMreg(round(fpos(7,2)/5)+1,round(fpos(7,1)/5)+1-round(5*csScale):round(fpos(7,1)/5)+1+round(5*csScale));
tnorm1D_FastBEMreg2_g2 = fnorm_FastBEMreg2(round(fpos(7,2)/5)+1,round(fpos(7,1)/5)+1-round(5*csScale):round(fpos(7,1)/5)+1+round(5*csScale));
tnorm1D_FTTCregfine_g2 = fnorm_fttcregfine(fpos(7,2),fpos(7,1)-5*imScale:fpos(7,1)+5*imScale);
tnorm1D_FastBEMregfine_g2 = fnorm_FastBEMregfine(fpos(7,2),fpos(7,1)-5*imScale:fpos(7,1)+5*imScale);
tnorm1D_FastBEMreg2fine_g2 = fnorm_FastBEMreg2fine(fpos(7,2),fpos(7,1)-5*imScale:fpos(7,1)+5*imScale);
tnorm1D_FastBEMn1fine_g2 = fnorm_FastBEMn1fine(fpos(7,2),fpos(7,1)-5*imScale:fpos(7,1)+5*imScale);
x1D_g2 = grid_mat_f(round(fpos(7,2)/5)+1,round(fpos(7,1)/5)+1-round(5*csScale):round(fpos(7,1)/5)+1+round(5*csScale),1);
x1Du_g2 = grid_mat_u(fpos(7,2),fpos(7,1)-5*imScale:fpos(7,1)+5*imScale,1);
...
r = 1.1; %
backgroundMask = sqrt((x_mat_f-(locMaxI(1,1)/5+1)).^2+(y_mat_f-(locMaxI(1,2)/5+1)).^2)>r ...
                    & sqrt((x_mat_f-(locMaxI(2,1)/5+1)).^2+(y_mat_f-(locMaxI(2,2)/5+1)).^2)>r ...
                    & sqrt((x_mat_f-(locMaxI(3,1)/5+1)).^2+(y_mat_f-(locMaxI(3,2)/5+1)).^2)>r ...
                    & sqrt((x_mat_f-(locMaxI(4,1)/5+1)).^2+(y_mat_f-(locMaxI(4,2)/5+1)).^2)>r ...
                    & sqrt((x_mat_f-(locMaxI(5,1)/5+1)).^2+(y_mat_f-(locMaxI(5,2)/5+1)).^2)>r ...
                    & sqrt((x_mat_f-(locMaxI(6,1)/5+1)).^2+(y_mat_f-(locMaxI(6,2)/5+1)).^2)>r ...
                    & sqrt((x_mat_f-(locMaxI(7,1)/5+1)).^2+(y_mat_f-(locMaxI(7,2)/5+1)).^2)>4.1 ...
                    & sqrt((x_mat_f-(locMaxI(8,1)/5+1)).^2+(y_mat_f-(locMaxI(8,2)/5+1)).^2)>r ...
                    & sqrt((x_mat_f-(locMaxI(9,1)/5+1)).^2+(y_mat_f-(locMaxI(9,2)/5+1)).^2)>r ...
                    & sqrt((x_mat_f-(locMaxI(10,1)/5+1)).^2+(y_mat_f-(locMaxI(10,2)/5+1)).^2)>r ...
                    & sqrt((x_mat_f-(locMaxI(11,1)/5+1)).^2+(y_mat_f-(locMaxI(11,2)/5+1)).^2)>r ...
                    & sqrt((x_mat_f-(locMaxI(12,1)/5+1)).^2+(y_mat_f-(locMaxI(12,2)/5+1)).^2)>r ...
                    & sqrt((x_mat_f-(locMaxI(13,1)/5+1)).^2+(y_mat_f-(locMaxI(13,2)/5+1)).^2)>r ...
                    & sqrt((x_mat_f-(locMaxI(14,1)/5+1)).^2+(y_mat_f-(locMaxI(14,2)/5+1)).^2)>4.1 ...
                    & sqrt((x_mat_f-(locMaxI(15,1)/5+1)).^2+(y_mat_f-(locMaxI(15,2)/5+1)).^2)>r ...
                    & sqrt((x_mat_f-(locMaxI(16,1)/5+1)).^2+(y_mat_f-(locMaxI(16,2)/5+1)).^2)>r ...
                    & sqrt((x_mat_f-(locMaxI(17,1)/5+1)).^2+(y_mat_f-(locMaxI(17,2)/5+1)).^2)>r ...
                    & sqrt((x_mat_f-(locMaxI(18,1)/5+1)).^2+(y_mat_f-(locMaxI(18,2)/5+1)).^2)>r ...
                    & sqrt((x_mat_f-(locMaxI(19,1)/5+1)).^2+(y_mat_f-(locMaxI(19,2)/5+1)).^2)>r;
backgroundMaskfine = sqrt((x_mat_u-(locMaxI(1,1))).^2+(y_mat_u-(locMaxI(1,2))).^2)>r ...
                    ...
                    & sqrt((x_mat_u-(locMaxI(7,1)/5+1)).^2+(y_mat_u-(locMaxI(7,2)/5+1)).^2)>4.1 ...
                    ...
                    & sqrt((x_mat_u-(locMaxI(14,1)/5+1)).^2+(y_mat_u-(locMaxI(14,2)/5+1)).^2)>4.1 ...
...
for k=1:nPeaks
    ...
    flocMaxAngle(k,jj+3) = acosd(recF_vecfine*orgF_vec'/...
        (norm(recF_vecfine)*norm(orgF_vec))); % in degree
    %radius for surroundings (2 um radius)
    if k==7 || k==12 
        r = 4.1;
    else
        r = 1.1;
...
subplot(4,5,7), surf(grid_matfine(:,:,1), grid_matfine(:,:,2), tnorm_FastBEMregfine), zlim([tmin-1 tmax+1]), view(0,90), shading interp; title(['FastBEM regularization L=' num2str(sol_mats_regfine.L)])
caxis([tmin-0.01 tmax+0.01]),set(gca, 'DataAspectRatio', [1,1,1]);
% hold on, line([4 16],[10 10],[tmax+1 tmax+1],'Color','y'), line([16 28],[15 15],[tmax+1 tmax+1],'Color','c')
xlim([xmin xmax])
ylim([ymin ymax])
...
hleg = legend('Original','FTTC','FastBEM0th','FastBEM2nd','FTTCfine','BEM0thfine','2ndfine','L1Norm','Location','North');
set(hleg,'FontAngle','italic','TextColor',[.3,.2,.1],'FontSize',6)
...
hleg2=legend('FTTC','0th','2nd','FTTCf','0thf','2ndf','1norm','Location','NorthWest');
...
%% save
regParams = [sol_mats_reg.L sol_mats_reg2.L sol_mats_regfine.L sol_mats_reg2fine.L sol_mats_1nfine.L];
