...
function [pSR,pSRerr,regParams] = calculateForceFineMeshImgScaled_fn(imScale,forceType,percentNoise,savePath,forceMeshFastBEM,forceMeshFastBEMfine,M_FastBEM,M_FastBEMfine,basisClassTablePath,basisClassTablePathFine)
...
fpos = [10*imScale,10*imScale;
        37*imScale,42*imScale;
        25*imScale,25*imScale;
        12*imScale,20*imScale;
        41*imScale,11*imScale];
fvec = [2000,-200;
        300,-800;
        450,-1800;
        600,-480;
...
force_x = force_x1+force_x2+force_x3+force_x4+force_x5;
force_y = force_y1+force_y2+force_y3+force_y4+force_y5;
toc
%% Forward solution
display('forward solution for fine solution...')
tic
[ux, uy]=fwdSolution(x_mat_u,y_mat_u,E,[],[],[],[],...
    @(x,y) assumedForceShifted(1,x,y,10*imScale,10*imScale,2000,-200,forceType,'largeFA')+...
    assumedForceShifted(1,x,y,37*imScale,42*imScale,300,-800,forceType,'largeFA')+...
    assumedForceShifted(1,x,y,25*imScale,25*imScale,450,-1800,forceType,'smallFA')+...
    assumedForceShifted(1,x,y,12*imScale,20*imScale,600,-480,forceType,'smallFA')+...
    assumedForceShifted(1,x,y,41*imScale,11*imScale,-1600,800,forceType,'largeFA'),...
    @(x,y) assumedForceShifted(2,x,y,10*imScale,10*imScale,2000,-200,forceType,'largeFA')+...
    assumedForceShifted(2,x,y,37*imScale,42*imScale,300,-800,forceType,'largeFA')+...
    assumedForceShifted(2,x,y,25*imScale,25*imScale,450,-1800,forceType,'smallFA')+...
    assumedForceShifted(2,x,y,12*imScale,20*imScale,600,-480,forceType,'smallFA')+...
    assumedForceShifted(2,x,y,41*imScale,11*imScale,-1600,800,forceType,'largeFA'),'fft',[],meshPtsFwdSol);
toc
...
[ux_f, uy_f]=fwdSolution(x_mat_f,y_mat_f,E,[],[],[],[],...
    @(x,y) assumedForceShifted(1,x,y,10*csScale,10*csScale,2000,-200,forceType,'largeFA')+...
    assumedForceShifted(1,x,y,37*csScale,42*csScale,300,-800,forceType,'largeFA')+...
    assumedForceShifted(1,x,y,25*csScale,25*csScale,450,-1800,forceType,'smallFA')+...
    assumedForceShifted(1,x,y,12*csScale,20*csScale,600,-480,forceType,'smallFA')+...
    assumedForceShifted(1,x,y,41*csScale,11*csScale,-1600,800,forceType,'largeFA'),...
    @(x,y) assumedForceShifted(2,x,y,10*csScale,10*csScale,2000,-200,forceType,'largeFA')+...
    assumedForceShifted(2,x,y,37*csScale,42*csScale,300,-800,forceType,'largeFA')+...
    assumedForceShifted(2,x,y,25*csScale,25*csScale,450,-1800,forceType,'smallFA')+...
    assumedForceShifted(2,x,y,12*csScale,20*csScale,600,-480,forceType,'smallFA')+...
    assumedForceShifted(2,x,y,41*csScale,11*csScale,-1600,800,forceType,'largeFA'),'fft',[],meshPtsFwdSol);
...
[fx_FastBEMregfine,fy_FastBEMregfine,~,~,M_FastBEMfine,~,~,~,sol_mats_regfine]...
    ...
    E,L,[],[],'fast',meshPtsFwdSol,...
    'QR','basisClassTblPath',basisClassTablePathFine,'imgRows',ymax,'imgCols',xmax,'fwdMap',M_FastBEMfine);
...
% For force 1
tnorm1D_org_g1 = fnorm_org(10*imScale,10*imScale-5*imScale:10*imScale+5*imScale);
tnorm1D_FTTCreg_g1 = fnorm_fttcreg(round(10*csScale)+1,round(10*csScale)+1-round(5*csScale):round(10*csScale)+1+round(5*csScale));
tnorm1D_FastBEMreg_g1 = fnorm_FastBEMreg(round(10*csScale)+1,round(10*csScale)+1-round(5*csScale):round(10*csScale)+1+round(5*csScale));
tnorm1D_FastBEMreg2_g1 = fnorm_FastBEMreg2(round(10*csScale)+1,round(10*csScale)+1-round(5*csScale):round(10*csScale)+1+round(5*csScale));
tnorm1D_FTTCregfine_g1 = fnorm_fttcregfine(10*imScale,10*imScale-5*imScale:10*imScale+5*imScale);
tnorm1D_FastBEMregfine_g1 = fnorm_FastBEMregfine(10*imScale,10*imScale-5*imScale:10*imScale+5*imScale);
tnorm1D_FastBEMreg2fine_g1 = fnorm_FastBEMreg2fine(10*imScale,10*imScale-5*imScale:10*imScale+5*imScale);
x1D_g1 = grid_mat_f(round(10*csScale)+1,round(10*csScale)+1-round(5*csScale):round(10*csScale)+1+round(5*csScale),1);
x1Du_g1 = grid_mat_u(10*imScale,10*imScale-5*imScale:10*imScale+5*imScale,1);
...
tnorm1D_org_g2 = fnorm_org(42*imScale,37*imScale-5*imScale:37*imScale+5*imScale);
tnorm1D_FTTCreg_g2 = fnorm_fttcreg(round(42*csScale)+1,round(37*csScale)+1-round(5*csScale):round(37*csScale)+1+round(5*csScale));
tnorm1D_FastBEMreg_g2 = fnorm_FastBEMreg(round(42*csScale)+1,round(37*csScale)+1-round(5*csScale):round(37*csScale)+1+round(5*csScale));
tnorm1D_FastBEMreg2_g2 = fnorm_FastBEMreg2(round(42*csScale)+1,round(37*csScale)+1-round(5*csScale):round(37*csScale)+1+round(5*csScale));
tnorm1D_FTTCregfine_g2 = fnorm_fttcregfine(42*imScale,37*imScale-5*imScale:37*imScale+5*imScale);
tnorm1D_FastBEMregfine_g2 = fnorm_FastBEMregfine(42*imScale,37*imScale-5*imScale:37*imScale+5*imScale);
tnorm1D_FastBEMreg2fine_g2 = fnorm_FastBEMreg2fine(42*imScale,37*imScale-5*imScale:37*imScale+5*imScale);
x1D_g2 = grid_mat_f(round(42*csScale)+1,round(37*csScale)+1-round(5*csScale):round(37*csScale)+1+round(5*csScale),1);
x1Du_g2 = grid_mat_u(42*imScale,37*imScale-5*imScale:37*imScale+5*imScale,1);
...
tnorm1D_org_g3 = fnorm_org(25*imScale,25*imScale-5*imScale:25*imScale+5*imScale);
tnorm1D_FTTCreg_g3 = fnorm_fttcreg(round(25*csScale)+1,round(25*csScale)+1-round(5*csScale):round(25*csScale)+1+round(5*csScale));
tnorm1D_FastBEMreg_g3 = fnorm_FastBEMreg(round(25*csScale)+1,round(25*csScale)+1-round(5*csScale):round(25*csScale)+1+round(5*csScale));
tnorm1D_FastBEMreg2_g3 = fnorm_FastBEMreg2(round(25*csScale)+1,round(25*csScale)+1-round(5*csScale):round(25*csScale)+1+round(5*csScale));
tnorm1D_FTTCregfine_g3 = fnorm_fttcregfine(25*imScale,25*imScale-5*imScale:25*imScale+5*imScale);
tnorm1D_FastBEMregfine_g3 = fnorm_FastBEMregfine(25*imScale,25*imScale-5*imScale:25*imScale+5*imScale);
tnorm1D_FastBEMreg2fine_g3 = fnorm_FastBEMreg2fine(25*imScale,25*imScale-5*imScale:25*imScale+5*imScale);
x1D_g3 = grid_mat_f(round(25*csScale)+1,round(25*csScale)+1-round(5*csScale):round(25*csScale)+1+round(5*csScale),1);
x1Du_g3 = grid_mat_u(25*imScale,25*imScale-5*imScale:25*imScale+5*imScale,1);
...
tnorm1D_org_g4 = fnorm_org(20*imScale,12*imScale-5*imScale:12*imScale+5*imScale);
tnorm1D_FTTCreg_g4 = fnorm_fttcreg(round(20*csScale)+1,round(12*csScale)+1-round(5*csScale):round(12*csScale)+1+round(5*csScale));
tnorm1D_FastBEMreg_g4 = fnorm_FastBEMreg(round(20*csScale)+1,round(12*csScale)+1-round(5*csScale):round(12*csScale)+1+round(5*csScale));
tnorm1D_FastBEMreg2_g4 = fnorm_FastBEMreg2(round(20*csScale)+1,round(12*csScale)+1-round(5*csScale):round(12*csScale)+1+round(5*csScale));
tnorm1D_FTTCregfine_g4 = fnorm_fttcregfine(20*imScale,12*imScale-5*imScale:12*imScale+5*imScale);
tnorm1D_FastBEMregfine_g4 = fnorm_FastBEMregfine(20*imScale,12*imScale-5*imScale:12*imScale+5*imScale);
tnorm1D_FastBEMreg2fine_g4 = fnorm_FastBEMreg2fine(20*imScale,12*imScale-5*imScale:12*imScale+5*imScale);
x1D_g4 = grid_mat_f(round(20*csScale)+1,round(12*csScale)+1-round(5*csScale):round(12*csScale)+1+round(5*csScale),1);
x1Du_g4 = grid_mat_u(20*imScale,12*imScale-5*imScale:12*imScale+5*imScale,1);
...
for jj=1:nMethods/2 
    for k=1:nPeaks
        ...
        flocMax(k,jj+nMethods/2) = tnormd(locMaxI(k,1),locMaxI(k,2),jj);
...
h1 = figure;
hold off
set(h1, 'Position', [100 100 1500 900])
%% Heatmaps
tmin = min(min(tnorm_org))-500;
tmax = max(max(tnorm_org))*1.4;
colormap('jet');
subplot(3,5,4), surf(grid_matfine(:,:,1), grid_matfine(:,:,2), tnorm_org,'FaceColor','interp',...
...
subplot(3,5,1), surf(grid_matfine(:,:,1), grid_matfine(:,:,2), tnorm_fttcregfine), zlim([tmin-1 tmax+1]), view(0,90), shading interp; title('FTTC regularization')
caxis([tmin-0.01 tmax+0.01]),set(gca, 'DataAspectRatio', [1,1,1]);
% hold on, line([4 16],[10 10],[tmax+1 tmax+1],'Color','y'), line([16 28],[15 15],[tmax+1 tmax+1],'Color','c')
xlim([xmin xmax])
ylim([ymin ymax])

subplot(3,5,2), surf(grid_matfine(:,:,1), grid_matfine(:,:,2), tnorm_FastBEMregfine), zlim([tmin-1 tmax+1]), view(0,90), shading interp; title('FastBEM regularization')
caxis([tmin-0.01 tmax+0.01]),set(gca, 'DataAspectRatio', [1,1,1]);
% hold on, line([4 16],[10 10],[tmax+1 tmax+1],'Color','y'), line([16 28],[15 15],[tmax+1 tmax+1],'Color','c')
xlim([xmin xmax])
ylim([ymin ymax])

subplot(3,5,3), surf(grid_matfine(:,:,1), grid_matfine(:,:,2), tnorm_FastBEMreg2fine), zlim([tmin-1 tmax+1])
...
subplot(3,5,5), caxis([tmin-0.01 tmax+0.01]), axis off; colorbar('West')
%% profile plot
subplot(3,5,6), %for high peak
