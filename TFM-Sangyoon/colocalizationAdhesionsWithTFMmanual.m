function [forceNA,forceFC,forceFA,forceBG] = colocalizationAdhesionsWithTFMmanual( pathForTheMovieDataFile,outputPath,band,tmax,pstruct_NAcell)
...
%     plot(NA_man_detected(:,1),NA_man_detected(:,2),'m*')
    ...
    % Find local maxima in force map of the background 
    distOutEdge = bwdist(cropMask);
    bandOut_pix = round(1000/MD.pixelSize_);
    bandOutMask = distOutEdge <= bandOut_pix;
    bandOutMask = bandOutMask & iMask;
    
    fImg = locmax2d(tsMap, 15);
    bgLocMax = bandOutMask.*fImg;
    forceBG = bgLocMax(bgLocMax(:)>0);
