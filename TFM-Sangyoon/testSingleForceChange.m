function [meanDispErrorAdh,meanDispErrorBG,dispDetec,meanForceErrorAdh,meanForceErrorBG,peakForceRatio,forceDetec,beadsOnAdh] = testSingleForceChange(d,dataPath,storagePath,varargin)
% Input check
ip = inputParser;
ip.CaseSensitive = false;
ip.addOptional('solMethodBEM','QR', @ischar);
ip.addOptional('regParam',1e-7, @isnumeric);
ip.parse(varargin{:});
...
FFbackupPath = [dataPath filesep 'TFMPackage/forceFieldL1/forceField.mat'];
mkdir([dataPath filesep 'TFMPackage/forceFieldL1']);
save(FFbackupPath, 'forceField')
MD.getPackage(iPack).getProcess(4).run();
forceField=MD.getPackage(iPack).getProcess(4).loadChannelOutput;
mkdir([storagePath filesep 'TFMPackage/forceField']);
save([storagePath filesep 'TFMPackage/forceField/forceField.mat'], 'forceField')

save([storagePath filesep 'movieData.mat'], 'MD');
