function fem = elModelUpdatePDE(fem,varargin)
%ELMODELUPDATE Update the FEM structure that contains all the information
%              about the elastic equations. This function is adopted from
%              ELMODELUPDATE but uses functions in matlab PDE toolbox.
%
% SYNOPSIS : 
%    fem = elModelUpdatePDE(fem,'name1',value1,'name2',value2,...)
%
% INPUT :
...
%
%    geom : An geometry object that defines the geometry of the domain.
...
if strcmp(geomChanged,'yes')
   if strcmp(meshChanged,'yes')
      fem = elModelAssemblePDE(geom,mesh,options,fn,fp,ind,bndInd);
   else
      fem = elModelAssemblePDE(geom,[],options,fn,fp,ind,bndInd);
   end
   return;
else
   if strcmp(meshChanged,'yes')
