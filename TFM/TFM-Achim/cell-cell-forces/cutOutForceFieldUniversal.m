function [constrForceField]=cutOutForceFieldUniversal(forceField,imageFileList,path_cellCellForces,doIntp,ROI)
...
%!!!        % convert the force back to a stress to plot it with the
        ...
        if numCells==1
            % store these values:
            j=1;
            constrForceField{i}.cell{j}.mask      = boolean(constrForceField{i}.segmRes.maskDilated);
            constrForceField{i}.cell{j}.extMask   = boolean(constrForceField{i}.segmRes.mask); %what was the extMask good for?
            constrForceField{i}.cell{j}.innerMask = boolean(constrForceField{i}.segmRes.mask); % Since segmRes.mask exclude holes, the innerMask also exclude any holes!
            constrForceField{i}.cell{j}.center    = centerOfMass(constrForceField{i}.cell{j}.mask);
            constrForceField{i}.cell{j}.boundary  = curveCellCluster;
            constrForceField{i}.cell{j}.interface = [];%[NaN,NaN];
            ...
            constrForceField{i}.cell{j}.pos=constrForceField{i}.roi.pos;
            constrForceField{i}.cell{j}.vec=constrForceField{i}.roi.vec;
            
            constrForceField{i}.cell{j}.stats.resForce.pos    =   constrForceField{i}.cell{j}.center;
            constrForceField{i}.cell{j}.stats.resForce.vec    = - constrForceField{i}.errorSumForce.vec; % the residual force for a single cell, is the error force! The sign convetion is adopted from the real cell clusters        
            constrForceField{i}.cell{j}.stats.resForce.mag    =   constrForceField{i}.errorSumForce.mag; % see line above
            constrForceField{i}.cell{j}.stats.method          =   constrForceField{i}.errorSumForce.method;
            constrForceField{i}.cell{j}.stats.sumForceMag     =   constrForceField{i}.sumForceMagCl;

            % So far these interface can belong to more than one cell pair!
            constrForceField{i}.interface{j}.pos=[];%[NaN,NaN];
        else
            for j=1:numCells-1
                curveInterface=[];
