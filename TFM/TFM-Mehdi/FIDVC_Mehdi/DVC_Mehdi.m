function [u, cc] = DVC_Mehdi(I,sSize,sSpacing,DVCPadSize,ccThreshold)
...
I{1} = padarray(I{1},DVCPadSize,0,'both');
I{2} = padarray(I{2},DVCPadSize,0,'both');
sizeV = size(I{1});

% Initialize Mesh Variables
idx = cell(1,3);
for i = 1:3, idx{i} = (1+DVCPadSize(i)) : sSpacing(i) : (sizeV(i)-sSize(i)-DVCPadSize(i)+1); end
...
MTF=nu{3};
% [I,m,mSize,sSize,MTF,M,ccThreshold] = parseInputs(I,sSize,sSpacing,DVCPadSize,ccThreshold);
...
for k = 1:mSize_
    ...
    A = I{1}(m_{1}(k,:),m_{2}(k,:),m_{3}(k,:));
    B = I{2}(m_{1}(k,:),m_{2}(k,:),m_{3}(k,:));
