function [pos_f,force,forceMesh,M,pos_u,u,sol_coef,sol_mats]=...
    reg_FEM_TFM(grid_mat, displField, frame, yModu_Pa, pRatio, regParam, varargin)
...
ip.addParamValue('FEM',false,@islogical);
...
if FEM
    [fx,fy,x_out,y_out,M,pos_u,u,sol_coef,sol_mats] = ...
        FEM_force_reconstruction(displField(frame).pos(:,1),displField(frame).pos(:,2),...
        displField(frame).vec(:,1),displField(frame).vec(:,2),forceMesh,yModu_Pa,regParam,...
        [],[],'fast',meshPtsFwdSol,solMethodFEM,'wtBar',wtBar,'thickness',thickness,'useLcurve',useLcurve,...
...
elseif isempty(paxImage)
    [fx,fy,x_out,y_out,M,pos_u,u,sol_coef,sol_mats] = ...
        FEM_force_reconstruction(displField(frame).pos(:,1),displField(frame).pos(:,2),...
        displField(frame).vec(:,1),displField(frame).vec(:,2),forceMesh,yModu_Pa,regParam,...
        [],[],'fast',meshPtsFwdSol,solMethodFEM,'basisClassTblPath',basisClassTblPath,...
    ...
    [fx,fy,x_out,y_out,M,pos_u,u,sol_coef,sol_mats] = ...
        FEM_force_reconstruction(displField(frame).pos(:,1),displField(frame).pos(:,2),...
        displField(frame).vec(:,1),displField(frame).vec(:,2),forceMesh,yModu_Pa,regParam,...
        x_out,y_out,'slow',meshPtsFwdSol,solMethodFEM,'wtBar',wtBar,'imgRows',imgRows,...
