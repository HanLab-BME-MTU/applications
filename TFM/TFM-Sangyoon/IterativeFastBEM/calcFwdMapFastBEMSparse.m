function [M]=calcFwdMapFastBEMSparse(x_vec_u, y_vec_u, forceMesh, E,varargin)
...
% Initialize sparse M
M=sparse(2*numPts,2*numBasis);

% % here actually each basis function has to be evaluated twice, which means
% % it actually give two values:
% ux=sparse(length(x_vec_u),2*numBasis);
% uy=sparse(length(y_vec_u),2*numBasis);
...
for class=1:numClass
    ...
    for oneORtwo=1:2        
        if ~isempty(idBestMatch)
            ...
            ux_model_pix = sparse(scaleE*double(basisClassTbl(idBestMatch).uSol.comp(oneORtwo).ux));
            uy_model_pix = sparse(scaleE*double(basisClassTbl(idBestMatch).uSol.comp(oneORtwo).uy));
            ...
            [ux_model, uy_model, x_model, y_model]=fwdSolution(xrangeSol,yrangeSol,...
                E,xbd_min,xbd_max,ybd_min,ybd_max,...
                forceMesh.basisClass(class).basisFunc(oneORtwo).f_intp_x,...
                forceMesh.basisClass(class).basisFunc(oneORtwo).f_intp_y,...
                'fft','noIntp',meshPtsFwdSol);
...
if addAtLeastOneToTbl
    save(basisClassTblPath, 'basisClassTbl');
end
