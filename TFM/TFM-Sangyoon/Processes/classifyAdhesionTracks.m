function []=classifyAdhesionTracks(movieData,varargin)
...
iProc = movieData.getProcessIndex('AdhesionClassificationProcess',1,0);
%If the process doesn't exist, create it
if isempty(iProc)
    iProc = numel(movieData.processes_)+1;
    movieData.addProcess(AdhesionClassificationProcess(movieData));                                                                                                 
end

adhClassifyProc = movieData.processes_{iProc};
paramsIn = parseProcessParams(adhClassifyProc,paramsIn);
...
iChanSlave=paramsIn.iChanSlave;
...
iTheOtherReadProc = movieData.getProcessIndex('TheOtherChannelReadingProcess');
theOtherReadProc = movieData.getProcess(iTheOtherReadProc);
paramsInTOR=theOtherReadProc.funParams_;
outputFilePathMaster = [paramsInTOR.OutputDirectory filesep paramsInTOR.outputPath filesep iChanName];
outputFilePrev=strcat(outputFilePathMaster,filesep,'tracksNA.mat');
% See if you can use existing tracks
if exist(outputFilePrev,'file')
    disp('Loading tracksNA file...')
    tracksNAFile = load(outputFilePrev,'tracksNA');
    tracksNA = tracksNAFile.tracksNA;
else
    disp('TheOtherChannelReadingProcess must be run before running this.')
end

%% classification after colocalization
    pathForColocalization = outputFilePathMaster; %[movieData filesep 'Colocalization' filesep outputPath];
...
%     tracksNA = filterOutNonEmergingNA(tracksNA);
    ...
    [idGroup1,idGroup2,idGroup3,idGroup4,idGroup5,idGroup6,idGroup7,idGroup8,idGroup9]= classifyNascentAdhesionTracks(pathForColocalization,'tracksNA',tracksNA,'movieData',movieData,'iChan',iChanMaster,'iChanSlave',iChanSlave);
    %% filter with idFiltered
    idGroup1filtered = idGroup1 & idEmerging;
    idGroup2filtered = idGroup2;
    idGroup3filtered = idGroup3 & idEmerging;
    idGroup4filtered = idGroup4;
    idGroup5filtered = idGroup5;
    idGroup6filtered = idGroup6;
    idGroup7filtered = idGroup7 & idEmerging;
    idGroup8filtered = idGroup8;
    idGroup9filtered = idGroup9 & idEmerging;
...
%     stdIntenBeforeLevel = std(curTrack.ampTotal(curTrack.startingFrameExtraExtra:curTrack.startingFrameExtra));
    ...
    if ~isfield(tracksNA,'forceUncertainty')
        tracksNA = readForceUncertaintyFromTracks(movieData,'tracksNA',tracksNA);
    end
    % Collect fCfd
