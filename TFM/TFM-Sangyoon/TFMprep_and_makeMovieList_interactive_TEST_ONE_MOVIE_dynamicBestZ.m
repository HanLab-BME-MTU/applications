...
%% ==== USER SETTINGS ====
baseOutName = "TFMprep_dynamicBestZ_TEST_ONE_MOVIE";   % output folder name under each movDir
metric      = "tenengrad";
verbose     = true;

% ==== TEST MODE ====
% true  = process only ONE movie per condition for quick testing
% false = process all movies
testOneMovieOnly = true;
testMovieIndex   = 1;      % 1 = first movie in the folder after filtering
...
for i = 1:size(conds,1)
    ...
    % ---- TEST MODE: keep only one movie file ----
    if testOneMovieOnly
        assert(testMovieIndex >= 1 && testMovieIndex <= numel(mFiles), ...
            'testMovieIndex=%d is out of range. This folder has %d movie file(s).', ...
            testMovieIndex, numel(mFiles));
        fprintf('[TEST MODE] Processing only movie %d/%d: %s\n', ...
            testMovieIndex, numel(mFiles), mFiles(testMovieIndex).name);
        mFiles = mFiles(testMovieIndex);
    end
