...
if exist(outputFile,'file')
    ...
    tracksNA=colocalizationAdhesionsWithTFM(pathForTheMovieDataFile,band,tmax,false,false,[],...
        'onlyEdge',false,'outputPath',outputPath,'saveAnalysis',true,'matchWithFA',true);
end
if ~uptoColo
    ...
    plotIntensityForce(tracksNA(idGroup2f),fileStoreG2,false,false)
...
%     arrayfun(@(x) x.CCmaxLag,tracksNA(idGroup1f(idxLongLTG1)));
    ccLagG1 = arrayfun(@(x) x.CCmaxLag,tracksNA(idGroup1f));
    ...
    plotIntensityForce(tracksNA(idGroup1f(idxTooMuchLag(4))),[],false,true)
