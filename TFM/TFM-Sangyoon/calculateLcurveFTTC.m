function  [rho,eta,reg_corner,ireg_corner] = calculateLcurveFTTC(grid_mat,u,E,s, cluster_size, i_max, j_max, L,LcurveFactor)
...
%     LcurveFactor = 10;
    ...
    for i=1:length(alphas);
        ...
        Ginv_xx =k./(V).*(kx.^2.*((1-s)*V^2)+ky.^2.*(((-1+s)*V)^2))./(((-1+s)*V)^2);
        Ginv_xx =k/V.*(kx.^2.-ky.^2.*((-1+s)))./(1-s);
...
%         residueU = ((ux-u(:,:,1)).^2+(uy-u(:,:,2)).^2).^0.5;
...
%     [reg_corner,ireg_corner,~]=regParamSelecetionLcurve(rho,eta,alphas,L);
    [reg_corner,ireg_corner,~]=regParamSelecetionLcurve(rho,eta,alphas,L,'manualSelection',true);
