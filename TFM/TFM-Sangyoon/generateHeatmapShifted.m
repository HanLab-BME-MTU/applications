...
forceMagMin = arrayfun(@(x) min((x.vec(:,1).^2 + x.vec(:,2).^2).^0.5),forceField);
tmax = quantile(forceMagMax,0.9);
...
for ii=iFrame
    if isstruct(displField)
        ...
        % robust mask: pos/vec ?? finite? ?? ??
        valid = all(isfinite(curDispVec),2) & all(isfinite(curDispPos),2);
        
        curDispVec = curDispVec(valid,:);
        curDispPos = curDispPos(valid,:);
