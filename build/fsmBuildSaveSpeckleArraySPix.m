function speckleArray=fsmBuildSaveSpeckleArraySPix(cM,outFileList,xmin,xmax,firstIndex,strg,noiseParams,threshold,factors,W,userPath,sigma)
% fsmBuildSaveSpeckleArray fills the speckleArray structure containing all speckle information extracted from the analysis of 
%                          an FSM stack. (Sub-pixel version).
%
% SYNOPSIS   speckleArray=fsmBuildSaveSpeckleArraySPix(cM,outFileList,xmin,xmax,firstIndex,strg,noiseParams,threshold,factors,userPath,sigma)
%
...
speckleArray=struct(...
    ...
    'spPos',       zeros(lengthSpeckleArray,2),...          % Double to accommodate subpixel postion.
...
for c1=1:tp                                    % Cycle through timepoints
    ...
    % Current
    eval(['load ',userPath,filesep,'cands',filesep,'cands',indxStr,'_spa','.mat;']); % Cands workspace
    cands = candsSP;
    candsS=cands; % Structure as is
    Lmax=[cands.Lmax]; 
    allCandsPos=reshape(Lmax,2,length(Lmax)/2)'; % All local maximum positions
    if ~isempty(cands)
        insCands=cands([cands.status]==0); % Cands structure containing only insignificant loc max
        clear Lmax;
        Lmax=[insCands.Lmax]; 
    ...
    if c1>1
        ...
        if ~isempty(cands)
            insCandsB=cands([cands.status]==0); % Cands structure containing only insignificant loc max
            clear Lmax;
            Lmax=[insCandsB.Lmax]; 
    ...
    if c1<tp
        ...
        if ~isempty(cands)
            insCandsD=cands([cands.status]==0); % Cands structure containing only insignificant loc max
            clear Lmax;
            Lmax=[insCandsD.Lmax]; 
        ...
        for c3=1:sp                              % Cycle through speckles (per template and timepoint)
            ...
            if s~=-1
                ...
                if b==0 && d==0
                    ...
                    speckleArray.spPos(s,1:2)    = coords'; 
                ...
                if b~=0 && d==0
                    ...
                    speckleArray.spPos(b,1:2)    = spPos';            
                ...
                if b==0 && d~=0
                    ...
                    speckleArray.spPos(d,1:2)    = spPos';         
...
if ~isempty(y) && ~isempty(x)
    if y~=1
        ...
        if length(x)>1
            ...
            for i=1:length(x)
                candsEvPosInt1 = max(1,min(imgHeight,candsEvPos(x(i),1)));
                candsEvPosInt2 = max(1,min(imgWidth,candsEvPos(x(i),2)));
                intensity(i)=img(candsEvPosInt1,candsEvPosInt2);
            end
            for i=1:length(x)
    ...
    lmPosInt1 = max(1,min(imgHeight,floor(lmPos(1)+0.5)));
    lmPosInt2 = max(1,min(imgWidth,floor(lmPos(2)+0.5)));
    intensity=img(lmPosInt1,lmPosInt2);
