function [growthPhases, shrinkagePhases] = getMTSwitchingPoints(dataListG,timePoints)
%GETMTSWITCHINGPOINTS extracts trajectory directional switching points starting with the output of trajectoryAnalysis
%
%INPUT  dataListG: The field
%                  trajectoryDescription.individualStatistics(i).dataListGroup 
%                  from the output of trajectoryAnalysis.
%       timePoints: List of available time points in a trajectory (i.e.
%                   time points where measurements are not missing).
%
%OUTPUT growthPhases:    Array with 2 columns indicating the start and end
%                        time points of growth phases.
%       shrinkagePhases: Array with 2 columns indicating the start and end
%                        time point of shrinkage phases.
% 
%REMARKS This is a quick hack modification of Jonas's code.
%
...
constants.TESTPROBABILITY = 0.9500;
constants.PROB2SIDES = 0.9750;
constants.PROBOUTLIER = 0.7500;
constants.PROBF = 0.7000;
constants.STRATEGY = 1;
constants.MINLENGTH = 2;
constants.MAXDELETED = 0;
constants.MINTRAJECTORYLENGTH = 3;
constants.DEBUG = [];
constants.DOCLUSTER = 0;
constants.CLUSTERMIN = 1;
constants.CLUSTERMAX = 5;
constants.CLUSTERIND = 0;
constants.CLUSTERTRY = 10;
constants.CLUSTERMINWEIGHT = 0.0500;
...
if ~isempty(growthIdx)
    ...
    %KJ
    growthPhases = [dataListG(growthGroups(:,1),1) dataListG(growthGroups(:,2),2)];
    growthPhases(:,1) = timePoints(growthPhases(:,1));
    growthPhases(:,2) = timePoints(growthPhases(:,2));
...
if ~isempty(shrinkageIdx)
    ...
    %KJ
    shrinkagePhases = [dataListG(shrinkageGroups(:,1),1) dataListG(shrinkageGroups(:,2),2)];
    shrinkagePhases(:,1) = timePoints(shrinkagePhases(:,1));
    shrinkagePhases(:,2) = timePoints(shrinkagePhases(:,2));
