...
for i = selectedLabels
    ...
    switch labelCase(i)
        ...
        case 13 %diffusion (uncorrected)
            ...
            [data,label,legendText,selectedTags{i},stats] = adgui_calcPlotData_tagDiffusion(handles,anaDat,xyzList(i),legendText,0);
        ...
        case 16 %diffusion (centroid corrected)
            ...
            [data,label,legendText,selectedTags{i},stats] = adgui_calcPlotData_tagDiffusion(handles,anaDat,xyzList(i),legendText,1);
        ...
        case 19 %timesteps for diffusion analysis
            ...
            %there will be n data points, separated by the average
            %timeLapse -> make a vector 1:n multiplied by a factor
            
            %use this only to count
            timePoints = cat(1,anaDat.timePoint);
            
            %timepoints2secondsFactor
            timeFactor = dataProperties.timeLapse;
            
            nTimePoints = max(floor(length(timePoints)/2),length(timePoints)-21);
            
            eval(['plotData.',xyzList(i),'Data = timeFactor*[1:nTimePoints]'';']);
            eval(['plotData.',xyzList(i),'_Label = ''Time [sec]'';']);
            eval(['plotData.',xyzList(i),'Stats = [];']);
       ...
       case 21 %velocity (tag centered)
        ...
        case 22 %diffusion (tag centered)
            ...
            [data,label,legendText,selectedTags{i},stats] = adgui_calcPlotData_tagDiffusion(handles,anaDat,xyzList(i),legendText,2);
        ...
        case 23 %diffusion (1-D)
            
            [data,label,legendText,selectedTags{i},stats] = adgui_calcPlotData_tagDiffusion(handles,anaDat,xyzList(i),legendText,3);
