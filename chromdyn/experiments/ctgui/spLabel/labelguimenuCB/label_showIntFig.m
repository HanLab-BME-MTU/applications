...
if ~isempty(figHandles)
    ...
    uh = uicontrol('Style','pushbutton',...
        'Tag','updateIntFig_PB',...
        'Position',[5,5,120,23],...
        'Callback','label_updateIntFigCB(gcbo,[],guidata(gcbo))',...
        'String','update tag intensities');
    set(uh,'Units','characters');
    % add showTimeButton
    uh2 = uicontrol('Style','togglebutton',...
        'Tag','showTimeIntFig_PB',...
        'Position',[130,5,60,23],...
        'Callback','label_updateIntFigCB(gcbo,[],guidata(gcbo))',...
        'String','show time');
    % store the button handle. First is the button, second the line
    figHandles.showTimeH = [uh2, 0];
    guidata(figH,figHandles);
end
