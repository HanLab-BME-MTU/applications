...
% load defaults
mammalianCostMatricesDef;
% convert idlist
 movieInfo = idlist2twgcInput(idlist);
 
 % run trackWithGapClosing
...
% write back into idlist
idlistOld = idlist;
nTracks = size(trackedFeatureNum,1);
clear idlist
    idlist(1:nTimepoints) = struct('linklist',zeros(nTracks,12),'info',[],...
    'stats',[],'trackInit',[], 'centroid', []);
for t = 1:nTimepoints
    
    if any(t==goodTimes')
    
    linkedIdxL = (trackedFeatureNum(:,t)>0);
    
    % write coordinates
    idlist(t).linklist(linkedIdxL,:) = idlistOld(t).linklist(trackedFeatureNum(linkedIdxL,t),:);
    
    % write tagNumbers, timepoint, estimatedIdx
    idlist(t).linklist(:,1) = t;
    idlist(t).linklist(:,4) = (1:nTracks)';
    idlist(t).linklist(~linkedIdxL,3) = 1;
    idlist(t).linklist(~linkedIdxL,5) = 3;
