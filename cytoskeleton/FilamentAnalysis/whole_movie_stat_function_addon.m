function Whole_movie_stat_cell = whole_movie_stat_function_addon(movieData)
...
if(exist([FilamentSegmentationProcessOutputDir, filesep, 'whole_movie_stat.mat'],'file')>0)
    Existing_Whole_movie_stat_cell = load([FilamentSegmentationProcessOutputDir, filesep, 'whole_movie_stat.mat'],'Whole_movie_stat_cell');
    Existing_Whole_movie_stat_cell = Existing_Whole_movie_stat_cell.Whole_movie_stat_cell;
    
    % check if the existing whole movie file include the currently
    % selected channel    
    for iChannel = 1:numel(movieData.channels_)
        if(numel(Existing_Whole_movie_stat_cell)>=iChannel)
            if(~isempty(Existing_Whole_movie_stat_cell{iChannel}))
                flag_complete(iChannel)=1;
            end
        end
    end
end
