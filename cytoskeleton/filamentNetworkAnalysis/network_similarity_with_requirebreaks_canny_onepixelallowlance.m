...
original_SM_current_seg = SM_current_seg;
...
if(~isempty(MT_current_model) &&  length(break_list)>0  )
    ...
    for iBreak = 1 : length(break_list)
        ...
        if(begin_x==end_x && begin_y==end_y)
            ...
            if(begin_y-1>0 && begin_y+1<=800 && begin_x-1>0 && begin_x+1<=800)
            ...
            if(begin_y-1>0 && begin_y+1<=800 && begin_x-1>0 && begin_x+1<=800)
                if(end_y-1>0 && end_y+1<=800 && end_x-1>0 && end_x+1<=800)
                    for iFila = 1 : model_length
...
for iP= 1 : 5
    for iT = 1 :5
        ...
        if(iP>=4 && iT>=4) % to save some time in comparing
            %% build a matched map display, including index; and find out the linked rate by seeing if the two ends are in the same filament index
            
            if(~isempty(MT_current_seg)  &&  length(break_list)>0 )
                
                 [labelMask_canny_seg, n_canny_fila] = bwlabel(MT_current_seg);
                             
                MT_seg_MT_BY_MT =cell(1,n_canny_fila);
                
                for iFila = 1 : n_canny_fila                    
                    MT_seg_MT_BY_MT{iFila} = (labelMask_canny_seg==iFila);
                end                
                ...
                Canny_Results{iP,iT}.linked_breaks_rate(iFrame) = mean(double(Matched_flag>0));
            else                
                Canny_Results{iP,iT}.linked_breaks_rate(iFrame) = NaN;
            end
        else
            Canny_Results{iP,iT}.linked_breaks_rate(iFrame) = NaN;
        end
