...
if(show_save_everything_flag==1)
    ...
    imagesc_white_nan(difference_map.similarity_scoremap_proximity,0,1);axis equal;axis off;
   CCC = colormap;
   if CCC(1,1)> CCC(end,1)
    ...
    imagesc_white_nan(difference_map.similarity_scoremap_alignment,0,1);axis equal;axis off;
  CCC = colormap;
  if CCC(1,1)> CCC(end,1)
    flip_colormap;
   end
   axis ij;
    ...
    imagesc_white_nan(difference_map.similarity_scoremap_combined,0,1);axis equal;axis off;
    CCC = colormap;
    if CCC(1,1)> CCC(end,1)
    flip_colormap;
    end
...
if(show_save_everything_flag==1)
    ...
    h6=figure(6); imagesc_nan_neg(difference_map.similarity_scoremap_combined,0);
    axis equal;axis off;
