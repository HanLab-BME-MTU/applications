...
function [res, theta, nms, scaleMap] = multiscaleLongSteerableDetector(img, M, varargin)
...
%     [ires{si}, itheta{si}, inms{si}] = steerableDetector(img, ip.Results.M, sigma(si));
    [ires{si}, itheta{si}, inms{si}] = longsteerableDetector(img, ip.Results.M, sigma(si));
