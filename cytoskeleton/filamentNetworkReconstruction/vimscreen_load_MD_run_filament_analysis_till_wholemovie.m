function this_MD = vimscreen_load_MD_run_filament_analysis_till_wholemovie(this_MD, Parameter_MD, varargin)
...
%               only till the whole movie stat, save and stop
...
if(~isempty(Parameter_MD))
    ...
    if(~ismember('whole_movie_filename',ip.UsingDefaults))
        ...
        if(run_with_new_param ==1)
            this_MD = filament_segmentation_wholemoviestat_save(this_MD,default_Params,whole_movie_filename);
        else
            this_MD = filament_segmentation_continue(this_MD,default_Params,whole_movie_filename);
        end
    else        
        if(run_with_new_param ==1)
            this_MD = filament_segmentation_wholemoviestat_save(this_MD,default_Params);
        else
