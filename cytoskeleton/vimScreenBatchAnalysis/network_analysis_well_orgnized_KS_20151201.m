...
%     end
%      if mod(i,12)==0
%     save([Group_ROOT_DIR,filesep,'movieList_plate_4_pnKS_gathered_1201.mat'],...
...
%               'mt_length_control_big_pool',      'vim_length_control_big_pool',     'vim_mean_st_control_big_pool',          'vim_sum_st_control_big_pool');
%               
%   median_vim_mean_st_ctrl = median(vim_mean_st_control_big_pool);
% median_vim_sum_st_ctrl = median(vim_sum_st_control_big_pool);
% 
%  
% median_vim_length_ctrl = median(vim_length_control_big_pool);
% median_mt_length_ctrl = median(mt_length_control_big_pool);
    ...
    for j = 1 :384
      ...
      %  [h,p,ksvalue]=kstest2(vim_mean_st_control_big_pool,vim_mean_st_all_cell{1,j});
        ...
        if(median_vim_mean_st_ctrl>all_ch1_median_valid(10,j))
            vim_mean_st_KS_toCtrl(1,j) = -abs(vim_mean_st_KS_toCtrl(1,j));
%         else
%             vim_mean_st_KS_toCtrl(1,j) = ksvalue;
    ...
    %    [h,p,ksvalue]=kstest2(vim_sum_st_control_big_pool,vim_sum_st_all_cell{1,j});
        ...
        if(median_vim_sum_st_ctrl>all_ch1_median_valid(9,j))
            vim_sum_st_KS_toCtrl(1,j) = -abs(vim_sum_st_KS_toCtrl(1,j));
%         else
%             vim_sum_st_KS_toCtrl(1,j) = ksvalue;
       ...
       %  [h,p,ksvalue]=kstest2(vim_length_control_big_pool,vim_length_all_cell{1,j});
         if(median_vim_length_ctrl>all_ch1_median_valid(2,j))
            vim_length_KS_toCtrl(1,j) = -abs(vim_length_KS_toCtrl(1,j));
%         else
%             vim_length_KS_toCtrl(1,j) = ksvalue;
         end
        
        % [h,p,ksvalue]=kstest2(mt_length_control_big_pool,mt_length_all_cell{1,j});
         if(median_mt_length_ctrl>all_ch2_median_valid(2,j))
            mt_length_KS_toCtrl(1,j) = -abs(mt_length_KS_toCtrl(1,j));
%         else
%            mt_length_KS_toCtrl(1,j) = ksvalue;
     ...
     if mod(j,24)==0 
         save(['movieList_plate_4_pn_KS_tocontrolbigpool.mat'],...
