function Z = vim_screen_fullplate_feature_KS_stat(plate_analysis_folder, , feature_index, control_index, feature_gathered_folder, ks_results_saving_folder)
...
% output: Z:                                  The Z index for each well
...
if(nargin<4)
    feature_gathered_folder = [plate_analysis_folder, filespe,'row_col_movieList'];
end

% if no output saving folder is given, use the movieList folder under plate analysis folder
if(nargin<5)
    ks_results_saving_folder = [plate_analysis_folder, filespe,'row_col_movieList'];
end
