function fsmDispScoreMaps(SCORE,fsmParam)
% fsmDispScoreMaps creates images with overlaid color-codes scores
% 
% SYNOPSIS   fsmDispScoreMaps(SCORE,fsmParam)
%
% INPUT      speckleArray : reduced speckle structure created in lifetime
...
if ~exist('firstImage','file')
   ...
   if exist(firstImage)
      [path,body,start,ext]=getFilenameBody(firstImage);
      dirName=[path filesep];
      fName=[body,start,ext];
   else
      % Load first image
      [fName,dirName] = uigetfile('*.tif','Select first image');
      if(isa(fName,'char') & isa(dirName,'char'))
