function [estimates, residual, estimatesSigma, BICvalue] = fitcurveMultiWeibullCDF_lsq(t, data, guessVec, fixVec)
    ...
    function F = multipleWeibullCDFun(params)
        ...
        if fixvar == 1
            for f=1:length(fixvector)
                ...
                elseif fixvector(f)==2
                    params(f) = abs(params(f));
                end % of if
            end % of for
        end % of if       
        ...
        [FittedCurve]=multiWeibullCDF(t,params);
