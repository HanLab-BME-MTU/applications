...
ip.addParameter('ProcessedTracks', [], @ischar);
ip.addParameter('LifetimeData', 'lifetimeData.mat', @ischar);
ip.addParameter('ReturnValidOnly', true, @islogical);
ip.addParameter('Cutoff_f', [], @isscalar);
ip.addParameter('ExcludeVisitors', true, @islogical);
ip.addParameter('Scale', false, @islogical);
ip.addParameter('DisplayScaling', false, @islogical);
ip.addParameter('RemoveOutliers', false, @islogical);
ip.addParameter('AmplitudeCorrectionFactor', [], @(x) isempty(x) || (size(x,1)==nd && size(x,2)==nCh));
ip.addParameter('Mask', false, @islogical);
ip.addParameter('Colormap', []);
ip.addParameter('AnalysisPath', 'Tracking', @ischar);
ip.parse(varargin{:});
