...
ip.addParameter('Cutoff_f', 5, @isscalar);
ip.addParameter('Print', false, @islogical);
ip.addParameter('Buffer', 5);
...
ip.addParameter('Colormap', jet(numel(data)));
ip.addParameter('CorrectObservationBias', true, @islogical);
ip.addParameter('InitDensity', 'mean', @(x) any(strcmpi(x, {'mean', 'median'})));
ip.addParameter('AmplitudeCorrection', []);
ip.addParameter('MaskPath', ['Detection' filesep 'cellmask.tif'], @ischar);
ip.addParameter('Binning', 1, @isposint);
ip.addParameter('CellArea', [], @isnumeric);
...
for i = 1:nd
    ...
    elseif exist([data(i).source ip.Results.MaskPath], 'file')==2
        ...
        lftRes.cellArea(i) = sum(mask(:)) * px^2 * 1e12; % in �m^2
    else
...
if any(strcmpi(ip.Results.Display, {'on','all'})) && ~ip.Results.PoolDatasets
    ...
    fprintf('Initiation density, all detected tracks:                  %.3f � %.3f [�m^-2 min^-1]\n', mean(lftRes.initDensityAll(:,1)), std(lftRes.initDensityAll(:,1)));
    fprintf('Initiation density, valid tracks (CCPs + CSs + visitors): %.3f � %.3f [�m^-2 min^-1]\n', mean(lftRes.initDensityIa(:,1)), std(lftRes.initDensityIa(:,1)));
    fprintf('Initiation density, CCPs:                                 %.3f � %.3f [�m^-2 min^-1]\n', mean(lftRes.initDensityCCP(:,1)), std(lftRes.initDensityCCP(:,1)));
    fprintf('Density of persistent structures:                         %.3f � %.3f [�m^-2]\n', mean(lftRes.persistentDensity), std(lftRes.persistentDensity));
