...
for ichannel = 1:numChannels
    ...
    if handles.markInitiation
        plot(axHandle,handles.lftInfo.Mat_xcoord(handles.lftInfo.Mat_disapp(:,handles.iframe)==1 & ...
            ...
            handles.lftInfo.Mat_lifetime(:,handles.iframe) >= lftMin ...
            & handles.lftInfo.Mat_lifetime(:,handles.iframe) <= lftMax,handles.iframe),'go');
    end
    if handles.markIternalization
        plot(axHandle,handles.lftInfo.Mat_xcoord(handles.lftInfo.Mat_disapp(:,handles.iframe)==-1 & ...
            ...
            handles.lftInfo.Mat_ycoord(handles.lftInfo.Mat_disapp(:,handles.iframe)==-1 & ...
            handles.lftInfo.Mat_lifetime(:,handles.iframe) >= lftMin ...
            & handles.lftInfo.Mat_lifetime(:,handles.iframe) <= lftMax,handles.iframe),'ro');
    end
    %Mark 'good gaps'
    if handles.markGap
        plot(axHandle,handles.lftInfo.Mat_xcoord(handles.lftInfo.Mat_status(:,handles.iframe)==4, handles.iframe), ...
            handles.lftInfo.Mat_ycoord(handles.lftInfo.Mat_status(:,handles.iframe)==4,handles.iframe),'wx');
    end
    %Mark 'bad gaps' and plot the trajectories of pits with bad gaps
    if handles.markBadTrajectories
        if handles.markGap
            plot(axHandle,handles.lftInfo.Mat_xcoord(handles.lftInfo.Mat_status(:,handles.iframe)==5, handles.iframe), ...
                handles.lftInfo.Mat_ycoord(handles.lftInfo.Mat_status(:,handles.iframe)==5,handles.iframe),'wx');
        end
        if handles.iframe == 1
