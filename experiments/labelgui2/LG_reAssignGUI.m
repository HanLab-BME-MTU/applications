...
% find good spots/tags - all but estimated single occurences. Correct for
% fusions
goodSpotsIdx = ...
    find(linklist(:,5) ~= 3 & linklist(:,3) ~= 3);
...
lc = idlistData.labelcolor(goodSpotsIdx);
pdText = [{'None'};lc(:)];
...
for iSpot = 1:nGoodSpots
    ...
    if linklist(currentSpot,3) == 1
        % estimated spot. Don't look for fusions, write parentheses around
        % name
        th = uicontrol(reAssignH, 'Style', 'text', ...
            'Units','Characters', 'Position', textPos,...
            'FontName','Helvetica','HorizontalAlignment','center',...
            'FontWeight','bold',...
            'String',sprintf('(%s)',idlistData.labelcolor{currentSpot}),...
        ...
        if length(currentTagList) > 1
            ...
            isFusedTag = (linklist(currentTagList,3) == 3);
            th = uicontrol(reAssignH, 'Style', 'text', ...
                'Units','Characters', 'Position', textPos,...
                'FontName','Helvetica','HorizontalAlignment','center',...
                'FontWeight','bold',...
                'String','fusion',...
                'BackgroundColor',colorMap(currentTagList(~isFusedTag),:),...
                'ForegroundColor',colorMap(currentTagList(isFusedTag),:));
            pdHandles(iSpot,1) = uicontrol(reAssignH, 'Style','popupmenu',...
                'Units','Characters', ...
                'Position', pd1Pos, 'FontName','Helvetica','String',pdText,...
                'Value',currentTagList(~isFusedTag) + 1,...
                'Callback','LG_rag_tagPD_Callback(gcbo,[],guidata(gcbo));');
            pdHandles(iSpot,2) = uicontrol(reAssignH, 'Style','popupmenu',...
                'Units','Characters', ...
                'Position', pd2Pos, 'FontName','Helvetica','String',pdText,...
