...
if ~isDeleted
    ...
    for iTag = 1:size(linklist,1)
        ...
        if linklist(iTag,2) > 0 && linklist(iTag,3) ~= 4
            ...
            % x y z
            spotInfo(iSpot).x = linklist(iTag,9);
            if isTracked
                spotInfo(iSpot).sx = sqrt(...
            ...
            spotInfo(iSpot).y = linklist(iTag,9);
            if isTracked
                spotInfo(iSpot).sy = sqrt(...
            ...
            spotInfo(iSpot).z = linklist(iTag,9);
            if isTracked
                spotInfo(iSpot).sz = sqrt(...
                    idlist(currentTime).info.totalQ_Pix((iTag-1)*3+3,(iTag-1)*3+3) *...
                    idlist(currentTime).linklist(iTag,12)) * pixelSize(21);
            else
                spotInfo(iSpot).sz = sqrt(...
...
if any(length(figureHandleOrPos) == [0,4]) || ~ishandle(figureHandleOrPos)
    ...
    for iSpot = maxSpots:-1:1
        ...
        th.spotName(iSpot) = uicontrol(figureHandle,'Style','text',...
            'Units','Characters', 'Position', [2,height-(9.5 + (iSpot-1)*4),46.2,1.7],...
            'FontName','Helvetica','HorizontalAlignment','left',...
            'FontWeight','normal',...
            'String','');
        th.spotPos(iSpot) = uicontrol(figureHandle,'Style','text',...
            'Units','Characters', 'Position', [2,height-(11.5 + (iSpot-1)*4),46.2,1.7],...
            'FontName','Helvetica','HorizontalAlignment','left',...
            'FontWeight','normal',...
            'String','');
    end
...
set(th.time,'String',sprintf(...
    'Time %4.2f  Sampling %1.2f  Loaded %i:%i',...
...
for iSpot = 1:maxSpots
    set(th.spotName(iSpot),'String','',...
            'uicontextmenu',[],...
            'UserData',[]);
    set(th.spotPos(iSpot),'String','',...
            'uicontextmenu',[],...
            'UserData',[]);
end
for iSpot = 1:nSpots
    % first line
    set(th.spotName(iSpot),'String',sprintf(...
        ['%s  ',repmat('%s ',length(spotInfo(iSpot).flag)),...
        ' SNR %3.2f  Amp %3.3f'],...
        spotInfo(iSpot).name,spotInfo(iSpot).flag{:},...
    ...
    set(th.spotPos(iSpot),'String',sprintf(...
        ['X %3.3f',char(177),'%1.3f  Y %3.3f',char(177),...
