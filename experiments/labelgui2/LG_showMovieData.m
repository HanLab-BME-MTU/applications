...
if ~isDeleted
    ...
    for iTag = 1:size(linklist,1)
        ...
        if linklist(iTag,2) > 0 && linklist(iTag,3) ~= 4
            ...
            % x y z
            spotInfo(iSpot).xmu = linklist(iTag,9);
            if isTracked
                spotInfo(iSpot).sxmu = sqrt(...
                    idlist(currentTime).info.totalQ_Pix((iTag-1)*3+1,(iTag-1)*3+1) *...
                    idlist(currentTime).linklist(iTag,12)) * pixelSize(1);
            else
                spotInfo(iSpot).sxmu = sqrt(...
                    idlist(currentTime).info.detectQ_Pix((iTag-1)*3+1,(iTag-1)*3+1) *...
                    idlist(currentTime).linklist(iTag,12)) * pixelSize(1);
            end
            spotInfo(iSpot).ymu = linklist(iTag,10);
            if isTracked
                spotInfo(iSpot).symu = sqrt(...
                    idlist(currentTime).info.totalQ_Pix((iTag-1)*3+2,(iTag-1)*3+2) *...
                    idlist(currentTime).linklist(iTag,12)) * pixelSize(1);
            else
                spotInfo(iSpot).symu = sqrt(...
                    idlist(currentTime).info.detectQ_Pix((iTag-1)*3+2,(iTag-1)*3+2) *...
                    idlist(currentTime).linklist(iTag,12)) * pixelSize(1);
            end
            spotInfo(iSpot).zmu = linklist(iTag,11);
            if isTracked
                spotInfo(iSpot).szmu = sqrt(...
                    idlist(currentTime).info.totalQ_Pix((iTag-1)*3+3,(iTag-1)*3+3) *...
                    idlist(currentTime).linklist(iTag,12)) * pixelSize(2);
            else
                spotInfo(iSpot).szmu = sqrt(...
                    idlist(currentTime).info.detectQ_Pix((iTag-1)*3+3,(iTag-1)*3+3) *...
                    idlist(currentTime).linklist(iTag,12)) * pixelSize(2);
            end

            spotInfo(iSpot).x = spotInfo(iSpot).xmu / pixelSize(1);
            spotInfo(iSpot).sx = spotInfo(iSpot).sxmu / pixelSize(1);
            spotInfo(iSpot).y = spotInfo(iSpot).ymu / pixelSize(1);
            spotInfo(iSpot).sy = spotInfo(iSpot).symu / pixelSize(1);
            spotInfo(iSpot).z = spotInfo(iSpot).zmu / pixelSize(2);
            spotInfo(iSpot).sz = spotInfo(iSpot).szmu / pixelSize(2);
...
if any(length(figureHandleOrPos) == [0,4]) || ~ishandle(figureHandleOrPos)
    ...
    th.time = uicontrol(figureHandle,'Style','text',...
        ...
        'Sampling: average time between frames. Loaded: list of movie frames in memory',...
        'String','');
    ...
    for iSpot = maxSpots:-1:1
        ...
        th.spotName(iSpot) = uicontrol(figureHandle,'Style','text',...
            'Units','Characters', 'Position', [2,height-(9.5 + (iSpot-1)*4),10,1.7],...
            'FontName','Helvetica','HorizontalAlignment','left',...
            'FontWeight','bold',...
            'String','');
        th.spotAmp(iSpot) = uicontrol(figureHandle,'Style','text',...
            'Units','Characters', 'Position', [12,height-(9.5 + (iSpot-1)*4),36.2,1.7],...
            'FontName','Helvetica','HorizontalAlignment','left',...
            'FontWeight','normal',...
            'String','');
        th.spotPos(iSpot) = uicontrol(figureHandle,'Style','text',...
            'Units','Characters', 'Position', [2,height-(11 + (iSpot-1)*4),46.2,1.7],...
            'FontName','Helvetica','HorizontalAlignment','left',...
            'FontWeight','normal',...
            'String','');
    end
...
set(th.time,'String',sprintf(...
    'Time %4.2f  Sampling %1.3f  Loaded %i:%i',...
