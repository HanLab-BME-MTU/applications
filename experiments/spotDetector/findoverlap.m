function cord=findoverlap(data,cord,dataProperties)
...
tsteps=size(data,5);
%wb_hdl=waitbar(0,'Checking for multiple spots...');
h= mywaitbar(0,[],tsteps,'Checking for multiple spots...');

for t=1:tsteps
    ncord=struct('sp',[],'mnint',[],'statistics',[],'parms',[]); %init now so that order of fields stays constant
    Q=[];
    chi=[];
    snr=[];
    ct=1;
...
%                 bg=0;
            ...
            for i= 1:numDist
                ...
                ncord.sp(ct).cord=[ncordList(i,2) ncordList(i,1) ncordList(i,3)];
                ncord.sp(ct).amp=ampList(i);
                ncord.sp(ct).bg=bg;
                ncord.sp(ct).mnint=mean(patch(:));
                % overlapping or single spot
                multi_1=numDist-i+1>size(cordList,1);
                ncord.sp(ct).mult=statistics.multi;
                %                    ncord.sp(ct).parms=statistics.parms;
                ct=ct+1;
        ...
        ncord.parms=statistics.parms;
        ncord.mnint=cord(t).mnint;
        if ~isempty(Q)
            ncord = testdistance(ncord,Q,chi,snr,dataProperties);
        else
