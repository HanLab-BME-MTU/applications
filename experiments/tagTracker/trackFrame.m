...
%init emptyCoord - a check to ensure that there are actually coordinates in
%our match
emptyCoord = 0;
...
for s=1:nspCur
    ...
    % check for problems with the match
    if isempty(match(s).coords) || emptyCoord == 1
        emptyCoord = 1;
    else
