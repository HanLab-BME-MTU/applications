function [sptPropInWindow,windowTrackAssign,trackWindowAssign,windowSize,analysisParam] = ...
    ...
    windowTrackAssign,trackWindowAssign,diffModeAnRes)
...
%                        so must be input if to be used.
%
...
sptPropInWindow = repmat(struct('directAll',[],'directPos',[],...
    'directNeg',[],'diffAnalysis',[],'modeAnalysis',[]),numBandRange,1);
...
for iBandRange = 1 : numBandRange
    ...
    [...
        ...
        fracMode1,fracMode2,fracMode3,fracMode4,fracModeUn,diffCoefDispSq] = ...
        ...
    ...
    modeAnalysis = struct('fracMode1',fracMode1,'fracMode2',fracMode2,'fracMode3',fracMode3,...
        'fracMode4',fracMode4,'fracModeUn',fracModeUn,'diffCoefDispSq',diffCoefDispSq);
    ...
    sptPropInWindow(iBandRange).modeAnalysis = modeAnalysis;
...
if numTracksCurrent == 0 %if there are no tracks in this window ...
    ...
    fracModeUn.mean(iType,iIncShifted) = ...
        length(find(isnan(trajModeCurrent)))/numTracksCurrent;
    fracModeUn.numPoints(iType,iIncShifted) = numTracksCurrent;
    
    %then classified tracks
    trajModeCurrent = trajModeCurrent(~isnan(trajModeCurrent));
    numTracksMode = length(trajModeCurrent);
    
    if numTracksMode > 0
        
        %diffusion mode 1
        fracMode1.mean(iType,iIncShifted) = ...
            length(find(trajModeCurrent==1))/numTracksMode;
        fracMode1.numPoints(iType,iIncShifted) = numTracksMode;
        
        %diffusion mode 2
        fracMode2.mean(iType,iIncShifted) = ...
            length(find(trajModeCurrent==2))/numTracksMode;
        fracMode2.numPoints(iType,iIncShifted) = numTracksMode;
        
        %diffusion mode 3
        fracMode3.mean(iType,iIncShifted) = ...
            length(find(trajModeCurrent==3))/numTracksMode;
        fracMode3.numPoints(iType,iIncShifted) = numTracksMode;
        
        %diffusion mode 4
        fracMode4.mean(iType,iIncShifted) = ...
            length(find(trajModeCurrent==4))/numTracksMode;
        fracMode4.numPoints(iType,iIncShifted) = numTracksMode;
        
    end
    
    %calculate average diffusion coefficient from the mode analysis
    tmpVec = trajDiffCoef2(tracksCurrent);
    tmpVec = tmpVec(~isnan(tmpVec));
    if ~isempty(tmpVec)
        diffCoefDispSq.mean(iType,iIncShifted) = mean(tmpVec);
        diffCoefDispSq.std(iType,iIncShifted) = std(tmpVec);
        diffCoefDispSq.numPoints(iType,iIncShifted) = length(tmpVec);
    end
