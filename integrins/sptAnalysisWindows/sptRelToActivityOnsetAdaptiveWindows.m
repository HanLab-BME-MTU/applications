...
sptPropInWindow = repmat(struct('directAll',[],'directPos',[],...
    'directNeg',[],'diffMSSAnalysis',[],'diffModeAnalysis',[]),numBandRange,1);
...
for iBandRange = 1 : numBandRange
    ...
    fracMode = struct('mean',NaN(numTypes,numWinFrames-1-minInc,numDiffMode+1),...
        'std',NaN(numTypes,numWinFrames-1-minInc,numDiffMode+1),...
        'numPoints',zeros(numTypes,numWinFrames-1-minInc,numDiffMode+1));
    
    diffCoefModeInd = struct('mean',NaN(numTypes,numWinFrames-1-minInc,numDiffMode),...
        'std',NaN(numTypes,numWinFrames-1-minInc,numDiffMode),...
        'numPoints',zeros(numTypes,numWinFrames-1-minInc,numDiffMode));
    ...
    sptPropInWindow(iBandRange).diffMSSAnalysis = diffAnalysis;
    sptPropInWindow(iBandRange).diffModeAnalysis = modeAnalysis;
...
if numTracksCurrent == 0 %if there are no tracks in this window ...
    ...
    %get number of modes
    numDiffMode = size(fracMode.mean,3) - 1;
    ...
    if ~isempty(tmpVec)
        diffCoefModeAll.mean(iType,iIncShifted) = mean(tmpVec);
        diffCoefModeAll.std(iType,iIncShifted) = std(tmpVec);
        diffCoefModeAll.numPoints(iType,iIncShifted) = length(tmpVec);
    end
    ...
    fracMode.mean(iType,iIncShifted,end) = ...
        length(find(isnan(trajModeCurrent)))/numTracksCurrent;
    fracMode.numPoints(iType,iIncShifted,end) = numTracksCurrent;
    ...
    if numTracksModeClass > 0
        ...
        fracMode.mean(iType,iIncShifted,1:end-1) = n/numTracksModeClass;
        fracMode.numPoints(iType,iIncShifted,1:end-1) = numTracksModeClass;
        
        %get standard deviations from bootstrap sample
        n = hist(modeVecBoot,1:numDiffMode)';
        fracBoot = n/numTracksModeClass;
        fracMode.std(iType,iIncShifted,1:end-1) = ...
        ...
        for iMode = 1 : numDiffMode
            ...
            diffCoefModeInd.mean(iType,iIncShifted,iMode) = mean(tmpTmpVec);
            diffCoefModeInd.std(iType,iIncShifted,iMode) = std(tmpTmpVec);
            diffCoefModeInd.numPoints(iType,iIncShifted,iMode) = length(tmpTmpVec);
        end
