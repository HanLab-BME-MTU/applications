function [f,g] = UGM_MRFLoss(wv,y,edgeStruct,infoStruct,inferFunc,varargin)
% [f,g] = UGM_MRFLoss(wv,y,edgeStruct,infoStruct,inferFunc,varargin)
...
if edgeStruct.useMex
    ...
    f = UGM_MRFLoss_subC(int32(y),nodePot,edgePot,int32(edgeEnds));
else
...
for i = 1:nInstances
    ...
    if nargout > 1
        ...
        if edgeStruct.useMex
            ...
            UGM_updateGradientC(gw,gv,X,Xedge,y(i,:),nodeBel,edgeBel,int32(nStates),int32(tieNodes),int32(tieEdges),int32(ising),int32(edgeEnds));
        else
            [gw,gv] = updateGradient(gw,gv,X,Xedge,y(i,:),nodeBel,edgeBel,infoStruct,edgeStruct);
        end
    end
