...
% where the individual vectors are differently angled.
    ...
    if nnan>0
        ...
        fprintf(['Warning: data set #:' num2str(k) ' has ' num2str(nnan) ' ''NaN'' values which ' waswere ' removed from all analysis and counts\n']);
    end
...
% Check for and deal with infinite values.
    ...
    if sum(infValues)%infFlag(k)
        ...
        fprintf(['Warning: data set #:' num2str(k) ' has ''inf'' values which ' waswere ' put in the end bin/s. The mean and std will not be displayed\n']);
...
%       That means we should use the colormap they gave
    ...
    elseif isnumeric(lineColor) && size(lineColor, 2) == 3 && size(lineColor, 1) == num2Plot
        lineStyleOrder = cell(1,num2Plot);
        for i = 1:num2Plot
            lineStyleOrder{i} = lineColor(i,:);
        end
...
if isinteger(x) % your done, duh, its an int.
    L=1;
    return; 
else
...
% histograms look wrong. Please take the time to investigate fully before
% commenting.
% 
