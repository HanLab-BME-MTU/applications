function [ stat ] = getFaceStatistics( skeletons, property, scale )
%plotFaceStatistics
%
...
if(isa(skeletons,'MovieList'))
    ...
    if(isfield(data,'Sf2'))
        stat(1) = lamins.functions.getFaceStatistics([data.Sf], property, scale);
        stat(2) = lamins.functions.getFaceStatistics([data.Sf2], property, scale);
        return;
    else
        skeletons = [data.Sf];
    end
...
switch(property)
    case 'EdgesPerFace'
        out = arrayfun(@(S) S.getEdgesPerFace,skeletons,'UniformOutput',false);
