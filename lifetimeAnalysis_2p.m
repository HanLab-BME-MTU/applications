...
N = length(kWeibull);
aVect = ones(1,N)/N;
lambdaVect = (1:N)/(N+1)*t(floor(end/2));
...
% Plot histogram & fit
figure;
plot(tc, histVect, 'k-', 'LineWidth', 1);
hold on;
...
% Fitted curve
[w W] = nWeibullCDF(tc-dt, kWeibull, lambdaVect, aVect);
...
for k = 1:N
    plot(tc, W(k,:), 'b', 'LineWidth', 1.5);
end
plot(tc, w, 'r-', 'LineWidth', 2);
axis([0 tc(end) 0 1]);
