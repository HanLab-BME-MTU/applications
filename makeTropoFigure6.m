function makeTropoFigure6(analysisPaths, outputDirectory)

outputDirectory = fullfile(outputDirectory, 'Fig6');
    ...
    for iMovie = 1:nMovies(iTM)
        ...
        for iProtState = 1:2
            ...
            for iBand = 1:nBands
                ...
                lifetime1 = trackInfos1{iBand}(:,3) - trackInfos1{iBand}(:,2);
                lifetime2 = trackInfos2{iBand}(:,3) - trackInfos2{iBand}(:,2);
                ...
                dataX{iProtState,iTM,iMovie,iBand} = lifetime1(ind);
                dataY{iProtState,iTM,iMovie,iBand} = lifetime2(link12(ind));
            end
        end
    end
end
...
for iProtState = 1:2
    ...
    for iTM = 1:numel(paths)
        ...
        for iBand = 1:nBands
            ...
            ind = find(M);
            [Y X] = ind2sub(size(M),ind);
            X = X * timeInterval;
            Y = Y * timeInterval;
            C = M(ind);
            
            scatter(X,Y, 20, C, 'Marker', '.');
            text((minX+maxX) * timeInterval / 2, (minY+maxY) * timeInterval / 2,['r = ' num2str(r)],'HorizontalAlignment','center', 'FontSize', 20);
            
            %set(gca,'XLim',[0 300]);
            %set(gca,'YLim',[0 300]);
            %axis equal;
            
            xlabel([tmNames{iTM} ' Lifetime (s)']);
            ylabel('Actin Lifetime (s)');
            
            fileName = [outputDirectory filesep 'Fig6_' panelNames{iProtState} ...
