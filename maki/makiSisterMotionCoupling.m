...
for iLabel = 1 : 3
    ...
    eval(['projectionCenter' label{iLabel,1} '(1:numSistersTotEff,1) = struct(''observations'',[]);'])
    ...
    eval(['angleCenter' label{iLabel,1} '(1:numSistersTotEff,1) = struct(''observations'',[]);'])
    ...
    eval(['dispMagCenter' label{iLabel,1} '(1:numSistersTotEff,1) = struct(''observations'',[]);'])
    ...
    if numSisters(iMovie) > 0
        ...
        for iSample = 1 : samplingPeriod
            ...
            for iSister = 1 : numSisters(iMovie)
                ...
                %calculate sister displacements between frames
                ...
                %calculate sister center positions over time
                coordsCenter = [(coords1(:,1:3)+coords2(:,1:3))/2 ...
                    sqrt(coords1(:,4:6).^2+coords2(:,4:6).^2)/2];
                
                %calculate center displacement between frames
                coordsCenterDiff = [coordsCenter(2:end,1:3)-coordsCenter(1:end-1,1:3) ...
                    sqrt(coordsCenter(2:end,4:6).^2+coordsCenter(1:end-1,4:6).^2)];
                ...
                projectionCenter = coordsCenterDiff(:,1:3) * sisterVec(1:end-1,1:3)';
                projectionCenter = diag(projectionCenter);
                ...
                dispMagCenter = sqrt(sum(coordsCenterDiff(:,1:3).^2,2));
                dispMagCenter = sign(projectionCenter) .* dispMagCenter;
                ...
                angleCenter = acos(projectionCenter ./ sqrt(sum(coordsCenterDiff(:,1:3).^2,2))) * 180 / pi;
                ...
                eval(['projectionCenter' label{iLabel,1} '(iGlobal' label{iLabel,1} ...
                    ').observations = projectionCenter;']) %um
                ...
                eval(['angleCenter' label{iLabel,1} '(iGlobal' label{iLabel,1} ...
                    ').observations = angleCenter;']) %degrees
                ...
                eval(['dispMagCenter' label{iLabel,1} '(iGlobal' label{iLabel,1} ...
                    ').observations = dispMagCenter;']) %um
...
for iLabel = 1 : 3
    ...
    eval(['centerProjDistr' label{iLabel,1} ' = [];'])
    eval(['centerProjParam' label{iLabel,1} ' = [];'])
    eval(['centerProjPosParam' label{iLabel,1} ' = [];'])
    eval(['centerProjNegParam' label{iLabel,1} ' = [];'])
    eval(['centerDispDistr' label{iLabel,1} ' = [];'])
    eval(['centerDispParam' label{iLabel,1} ' = [];'])
    eval(['centerDispPosParam' label{iLabel,1} ' = [];'])
    eval(['centerDispNegParam' label{iLabel,1} ' = [];'])
    eval(['centerAngleDistr' label{iLabel,1} ' = [];'])
    eval(['centerAngleParam' label{iLabel,1} ' = [];'])
    ...
    eval(['centerProjIndParam' label{iLabel,1} ' = NaN(numMovies,7);'])
    eval(['centerProjPosIndParam' label{iLabel,1} ' = NaN(numMovies,7);'])
    eval(['centerProjNegIndParam' label{iLabel,1} ' = NaN(numMovies,7);'])
    eval(['centerDispIndParam' label{iLabel,1} ' = NaN(numMovies,7);'])
    eval(['centerDispPosIndParam' label{iLabel,1} ' = NaN(numMovies,7);'])
    eval(['centerDispNegIndParam' label{iLabel,1} ' = NaN(numMovies,7);'])
    eval(['centerAngleIndParam' label{iLabel,1} ' = NaN(numMovies,7);'])
...
for iLabel = goodLabel
    ...
    eval(['sister1ProjParam' label{iLabel,1} ... %everything together
        ' = [nanmean(allValues) nanstd(allValues) ' ...
        'min(allValues) prctile(allValues,25) '...
        'prctile(allValues,50) prctile(allValues,75) '...
        'max(allValues)];']);
    allValuesTmp = allValues(allValues>0); %only positive values
    if ~isempty(allValuesTmp)
        eval(['sister1ProjPosParam' label{iLabel,1} ...
            ' = [nanmean(allValuesTmp) nanstd(allValuesTmp) ' ...
            'min(allValuesTmp) prctile(allValuesTmp,25) '...
            'prctile(allValuesTmp,50) prctile(allValuesTmp,75) '...
            'max(allValuesTmp)];']);
    end
    allValuesTmp = allValues(allValues<0); %only negative values
    if ~isempty(allValuesTmp)
        eval(['sister1ProjNegParam' label{iLabel,1} ...
            ' = [nanmean(allValuesTmp) nanstd(allValuesTmp) ' ...
            'min(allValuesTmp) prctile(allValuesTmp,25) '...
    ...
    eval(['sister1DispParam' label{iLabel,1} ... %everything together
        ' = [nanmean(allValues) nanstd(allValues) ' ...
        'min(allValues) prctile(allValues,25) '...
        'prctile(allValues,50) prctile(allValues,75) '...
        'max(allValues)];']);
    ...
    eval(['sister2ProjParam' label{iLabel,1} ... %everything together
        ' = [nanmean(allValues) nanstd(allValues) ' ...
        'min(allValues) prctile(allValues,25) '...
        'prctile(allValues,50) prctile(allValues,75) '...
        'max(allValues)];']);
    ...
    eval(['sister2DispParam' label{iLabel,1} ... %everything together
        ' = [nanmean(allValues) nanstd(allValues) ' ...
        'min(allValues) prctile(allValues,25) '...
        'prctile(allValues,50) prctile(allValues,75) '...
        'max(allValues)];']);
    ...
    %center projections
    eval(['allValues = vertcat(projectionCenter' label{iLabel,1} '.observations);']);
    allValues = allValues(:,1);
    eval(['centerProjDistr' label{iLabel,1} ' = allValues;']);
    eval(['centerProjParam' label{iLabel,1} ... %everything together
        ' = [nanmean(allValues) nanstd(allValues) ' ...
        'min(allValues) prctile(allValues,25) '...
        'prctile(allValues,50) prctile(allValues,75) '...
        'max(allValues)];']);
    ...
    eval(['allValues = vertcat(dispMagCenter' label{iLabel,1} '.observations);']);
    allValues = allValues(:,1);
    eval(['centerDispDistr' label{iLabel,1} ' = allValues;']);
    eval(['centerDispParam' label{iLabel,1} ... %everything together
        ' = [nanmean(allValues) nanstd(allValues) ' ...
        'min(allValues) prctile(allValues,25) '...
        'prctile(allValues,50) prctile(allValues,75) '...
        'max(allValues)];']);
    ...
    %center angles
    eval(['allValues = vertcat(angleCenter' label{iLabel,1} '.observations);']);
    allValues = allValues(:,1);
    eval(['centerAngleDistr' label{iLabel,1} ' = allValues;']);
    eval(['centerAngleParam' label{iLabel,1} ...
        ' = [nanmean(allValues) nanstd(allValues) min(allValues) ' ...
        'prctile(allValues,25) prctile(allValues,50) prctile(allValues,75) '...
        'max(allValues)];']);
    ...
    allValuesTmp = allValues(allValues>0);
    if ~isempty(allValuesTmp)
        eval(['sisterSepChangePosParam' label{iLabel,1} ...
            ' = [nanmean(allValuesTmp) nanstd(allValuesTmp) ' ...
            'min(allValuesTmp) prctile(allValuesTmp,25) '...
    ...
    for iMovie = 1 : numMovies
        ...
        if ~isempty(allValues)
            allValues = allValues(:,1);
            eval(['sister1ProjIndParam' label{iLabel,1} '(iMovie,:)' ...
                ' = [nanmean(allValues) nanstd(allValues) min(allValues) ' ...
                'prctile(allValues,25) prctile(allValues,50) prctile(allValues,75) '...
                'max(allValues)];']);
            allValuesTmp = allValues(allValues>0);
    ...
    for iMovie = 1 : numMovies
        ...
        if ~isempty(allValues)
            allValues = allValues(:,1);
            eval(['sister1DispIndParam' label{iLabel,1} '(iMovie,:)' ...
                ' = [nanmean(allValues) nanstd(allValues) min(allValues) ' ...
                'prctile(allValues,25) prctile(allValues,50) prctile(allValues,75) '...
                'max(allValues)];']);
            allValuesTmp = allValues(allValues>0);
    ...
    for iMovie = 1 : numMovies
        ...
        if ~isempty(allValues)
            allValues = allValues(:,1);
            eval(['sister2ProjIndParam' label{iLabel,1} '(iMovie,:)' ...
                ' = [nanmean(allValues) nanstd(allValues) min(allValues) ' ...
                'prctile(allValues,25) prctile(allValues,50) prctile(allValues,75) '...
                'max(allValues)];']);
            allValuesTmp = allValues(allValues>0);
    ...
    for iMovie = 1 : numMovies
        ...
        if ~isempty(allValues)
            allValues = allValues(:,1);
            eval(['sister2DispIndParam' label{iLabel,1} '(iMovie,:)' ...
                ' = [nanmean(allValues) nanstd(allValues) min(allValues) ' ...
                'prctile(allValues,25) prctile(allValues,50) prctile(allValues,75) '...
                'max(allValues)];']);
            allValuesTmp = allValues(allValues>0);
    ...
    %center projections
    for iMovie = 1 : numMovies
        eval(['allValues = vertcat(projectionCenter' label{iLabel,1} '(movieStartIndx' ...
            label{iLabel,1} '(iMovie):movieEndIndx' label{iLabel,1} '(iMovie)).observations);']);
        if ~isempty(allValues)
            allValues = allValues(:,1);
            eval(['centerProjIndParam' label{iLabel,1} '(iMovie,:)' ...
                ' = [nanmean(allValues) nanstd(allValues) min(allValues) ' ...
                'prctile(allValues,25) prctile(allValues,50) prctile(allValues,75) '...
                'max(allValues)];']);
            allValuesTmp = allValues(allValues>0);
            if ~isempty(allValuesTmp)
                eval(['centerProjPosIndParam' label{iLabel,1} '(iMovie,:)' ...
                    ' = [nanmean(allValuesTmp) nanstd(allValuesTmp) ' ...
                    'min(allValuesTmp) prctile(allValuesTmp,25) '...
                    'prctile(allValuesTmp,50) prctile(allValuesTmp,75) '...
                    'max(allValuesTmp)];']);
            ...
            allValuesTmp = allValues(allValues<0);
            if ~isempty(allValuesTmp)
                eval(['centerProjNegIndParam' label{iLabel,1} '(iMovie,:)' ...
                    ' = [nanmean(allValuesTmp) nanstd(allValuesTmp) ' ...
                    'min(allValuesTmp) prctile(allValuesTmp,25) '...
                    'prctile(allValuesTmp,50) prctile(allValuesTmp,75) '...
                    'max(allValuesTmp)];']);
    ...
    %center displacements
    for iMovie = 1 : numMovies
        eval(['allValues = vertcat(dispMagCenter' label{iLabel,1} '(movieStartIndx' ...
            label{iLabel,1} '(iMovie):movieEndIndx' label{iLabel,1} '(iMovie)).observations);']);
        if ~isempty(allValues)
            allValues = allValues(:,1);
            eval(['centerDispIndParam' label{iLabel,1} '(iMovie,:)' ...
                ' = [nanmean(allValues) nanstd(allValues) min(allValues) ' ...
                'prctile(allValues,25) prctile(allValues,50) prctile(allValues,75) '...
                'max(allValues)];']);
            allValuesTmp = allValues(allValues>0);
            if ~isempty(allValuesTmp)
                eval(['centerDispPosIndParam' label{iLabel,1} '(iMovie,:)' ...
                    ' = [nanmean(allValuesTmp) nanstd(allValuesTmp) ' ...
                    'min(allValuesTmp) prctile(allValuesTmp,25) '...
                    'prctile(allValuesTmp,50) prctile(allValuesTmp,75) '...
                    'max(allValuesTmp)];']);
            ...
            allValuesTmp = allValues(allValues<0);
            if ~isempty(allValuesTmp)
                eval(['centerDispNegIndParam' label{iLabel,1} '(iMovie,:)' ...
                    ' = [nanmean(allValuesTmp) nanstd(allValuesTmp) ' ...
                    'min(allValuesTmp) prctile(allValuesTmp,25) '...
                    'prctile(allValuesTmp,50) prctile(allValuesTmp,75) '...
                    'max(allValuesTmp)];']);
    ...
    %center angles
    for iMovie = 1 : numMovies
        eval(['allValues = vertcat(angleCenter' label{iLabel,1} '(movieStartIndx' ...
            label{iLabel,1} '(iMovie):movieEndIndx' label{iLabel,1} '(iMovie)).observations);']);
        if ~isempty(allValues)
            allValues = allValues(:,1);
            eval(['centerAngleIndParam' label{iLabel,1} '(iMovie,:)' ...
                ' = [nanmean(allValues) nanstd(allValues) min(allValues) ' ...
                'prctile(allValues,25) prctile(allValues,50) prctile(allValues,75) '...
                'max(allValues)];']);
        end
...
%initialization
for iLabel = 1 : 3
    eval(['projectionCrosscorr' label{iLabel,1} ' = NaN(2*maxLag+1,2);'])
    eval(['angleCrosscorr' label{iLabel,1} ' = NaN(2*maxLag+1,2);'])
    eval(['projection1Autocorr' label{iLabel,1} ' = NaN(maxLag+1,2);'])
    eval(['angle1Autocorr' label{iLabel,1} ' = NaN(maxLag+1,2);'])
    eval(['projection2Autocorr' label{iLabel,1} ' = NaN(maxLag+1,2);'])
    eval(['angle2Autocorr' label{iLabel,1} ' = NaN(maxLag+1,2);'])
...
for iLabel = goodLabel
    ...
    %projections
    eval(['[tmpCorr,errFlag] = crossCorr(projectionSis1' ...
        label{iLabel,1} ',projectionSis2' label{iLabel,1} ',maxLag);'])
    if ~errFlag
        eval(['projectionCrosscorr' label{iLabel,1} ' = tmpCorr;'])
    end

    %angles
    eval(['[tmpCorr,errFlag] = crossCorr(angleSis1' ...
        label{iLabel,1} ',angleSis2' label{iLabel,1} ',maxLag);'])
    if ~errFlag
        eval(['angleCrosscorr' label{iLabel,1} ' = tmpCorr;'])
    end

    %sister with itself
    
    %projections
    eval(['[tmpCorr,errFlag] = autoCorr(projectionSis1' ...
        label{iLabel,1} ',maxLag);'])
    if ~errFlag
        eval(['projection1Autocorr' label{iLabel,1} ' = tmpCorr;'])
    end

    %angles
    eval(['[tmpCorr,errFlag] = autoCorr(angleSis1' ...
        label{iLabel,1} ',maxLag);'])
    if ~errFlag
        eval(['angle1Autocorr' label{iLabel,1} ' = tmpCorr;'])
    end
    
    %projections
    eval(['[tmpCorr,errFlag] = autoCorr(projectionSis2' ...
        label{iLabel,1} ',maxLag);'])
    if ~errFlag
        eval(['projection2Autocorr' label{iLabel,1} ' = tmpCorr;'])
    end

    %angles
    eval(['[tmpCorr,errFlag] = autoCorr(angleSis2' ...
        label{iLabel,1} ',maxLag);'])
    if ~errFlag
        eval(['angle2Autocorr' label{iLabel,1} ' = tmpCorr;'])
    end
...
for iLabel = 1 : 3
    ...
    eval(['distribution = struct('...
        ...
        '''projectionCenter'',centerProjDistr' label{iLabel,1} ','...
        '''signedDispMagCenter'',centerDispDistr' label{iLabel,1} ','...
        '''angleCenter'',centerAngleDistr' label{iLabel,1} ',' ...
    ...
    eval(['meanStdMin25P50P75PMax.all = struct('...
        ...
        '''projectionCenter'',centerProjParam' label{iLabel,1} ','...
        '''projectionCenterPos'',centerProjPosParam' label{iLabel,1} ','...
        '''projectionCenterNeg'',centerProjNegParam' label{iLabel,1} ','...
        '''dispMagCenter'',centerDispParam' label{iLabel,1} ','...
        '''dispMagCenterPos'',centerDispPosParam' label{iLabel,1} ','...
        '''dispMagCenterNeg'',centerDispNegParam' label{iLabel,1} ','...
        '''angleCenter'',centerAngleParam' label{iLabel,1} ','...
    ...
    eval(['meanStdMin25P50P75PMax.indcell = struct('...
        ...
        '''projectionCenter'',centerProjIndParam' label{iLabel,1} ','...
        '''projectionCenterPos'',centerProjPosIndParam' label{iLabel,1} ','...
        '''projectionCenterNeg'',centerProjNegIndParam' label{iLabel,1} ','...
        '''dispMagCenter'',centerDispIndParam' label{iLabel,1} ','...
        '''dispMagCenterPos'',centerDispPosIndParam' label{iLabel,1} ','...
        '''dispMagCenterNeg'',centerDispNegIndParam' label{iLabel,1} ','...
        '''angleCenter'',centerAngleIndParam' label{iLabel,1} ','...
