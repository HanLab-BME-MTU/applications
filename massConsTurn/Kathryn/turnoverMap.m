function [polyDepoly,accumY,accumX,runInfo]=turnoverMap(imDir,anDir,nPairs2analyze,doNorm,winL)

% parameters for vector outlier detection
runInfo.thresh=50;
runInfo.nNeighborLowerLimit=3;
runInfo.dirDev=10;

runInfo.nPtsPerSide=4; % how many points on each side of the measurement window to use


% here's some stuff for testing purposes
synthetic=0; % refers to whether we're testing synthetic actin movies
...
%imDir='S:\scripps\analysis\thompson\syntheticActin\convergence\depoly\exp1\images\tifs';
%anDir='S:\scripps\analysis\thompson\syntheticActin\convergence\depoly\exp1\analysis';

%imDir=[pwd filesep 'exp5' filesep 'images' filesep 'tifs'];
%anDir=[pwd filesep 'exp5' filesep 'analysis'];
...
mkdir([runInfo.turnDir filesep 'polyDepolyMaps']);

runInfo.movieMin=10^6;
runInfo.movieMax=0;
...
for pairNumber=1:runInfo.nIm2an-1
    ...
    img0 = filterRegion(img0, cellMask0, blurKernel);
    img1 = filterRegion(img1, cellMask1, blurKernel);
    ...
    elseif yTLC>1
        ...
        warning('border problem in grid to calculate poly/depoly');
    end
    ...
    % poly, so take the second frame centers
    ...
    [uniqueE,nOccurences,whereIdx] = countEntries(pixIdx,1,0);
    ...
    runInfo.movieMin=nanmin(runInfo.movieMin,nanmin(polyDepoly(:)));
    runInfo.movieMax=nanmax(runInfo.movieMax,nanmax(polyDepoly(:)));
    ...
    indxStr1=sprintf(strg,pairNumber);
    saveas(gcf,[runInfo.turnDir,filesep,'isomorphicCellEdges' indxStr1 '.fig']);


    indxStr1=sprintf(strg,pairNumber);
    save([runInfo.turnDir filesep 'polyDepolyMaps' filesep 'polyDepoly' indxStr1 '.mat'],'polyDepoly');
    save([runInfo.turnDir filesep 'polyDepolyMaps' filesep 'vectorCoverageMask' indxStr1 '.mat'],'vectorCoverageMask');
