function [] = gefFig5_directionalityPC1ScreenHits()
...
% betaPixCntl = controlRepPC1;
% betaPixKD = geneRepPC1;
% NbetaPix = length(controlMeanPC1);
% nbetaPix = length(betaPixCntl);
% 
% load([dataDname '/SOS1/Speed_stats.mat']);
% sos1Cntl = controlRepPC1;
% sos1KD = geneRepPC1;
% Nsos1 = length(controlMeanPC1);
% nsos1 = length(sos1Cntl);

load([dataDname '/RHOA/Directionality_stats.mat']);
rhoaCntl = controlRepPC1;
rhoaKD = geneRepPC1;
Nrhoa = length(controlMeanPC1);
nrhoa = length(rhoaCntl);

load([dataDname '/ARHGEF18/Directionality_stats.mat']);
arhgef18Cntl = controlRepPC1;
arhgef18KD = geneRepPC1;
Narhgef18 = length(controlMeanPC1);
narhgef18 = length(arhgef18Cntl);

load([dataDname '/ARHGEF11/Directionality_stats.mat']);
arhgef11Cntl = controlRepPC1;
arhgef11KD = geneRepPC1;
Narhgef11 = length(controlMeanPC1);
narhgef11 = length(arhgef11Cntl);

load([dataDname '/ARHGEF28/Directionality_stats.mat']);
arhgef28Cntl = controlRepPC1;
arhgef28KD = geneRepPC1;
Narhgef28 = length(controlMeanPC1);
narhgef28 = length(arhgef28Cntl);

load([dataDname '/ARHGEF3/Directionality_stats.mat']);
arhgef3Cntl = controlRepPC1;
arhgef3KD = geneRepPC1;
...
set(haxes,'YTick',[-5,0,5]);
set(haxes,'YTickLabel',[-5,0,5]);
...
export_fig([figDname 'directionalityPC1ScreenHits_diff_legend.eps']);
legend off;
...
set(axisHandle,'Position',APosition,'box','off','XMinorTick','off','TickDir','out','YMinorTick','off','FontSize',fontsize,'LineWidth',2);
set(get(axisHandle,'XLabel'),'FontSize',fontsize); set(get(axisHandle,'YLabel'),'FontSize',fontsize);
export_fig([figDname 'directionalityPC1ScreenHits_diff.eps']);
hold off;
