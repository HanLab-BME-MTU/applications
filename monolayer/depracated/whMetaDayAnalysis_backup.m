...
for day = 1 : N
    ...
    for curGene = 1 : nGene
        ...
        geneDayDiff{nGeneDay}.diffMeanGeneToMeanControl = diffMeanGeneToMeanControl;
...
for iGeneDay = 1 : nGeneDay        
    ...
    for i = 1 : nsamples
        meanGeneDist = mean(Z(i,[1:i-1, i+1:nGene]));
        meanControlDist = mean(Z(i,[nGene+1:i-1, i+1:end]));
        clsWeights(i) = meanControlDist - meanGeneDist;
        clsLabels(i) = sign(clsWeights(i));
    end 
       
    pval = ranksum(clsWeights(1:nGene),clsWeights(nGene+1:end));
    geneDayDiff{iGeneDay}.pval = pval;
    geneDayDiff{iGeneDay}.svmAccuracy = [num2str(sum(clsLabels == labels)) '/' num2str(length(labels))];

    % Another measure: d / (std1 + std2)
    meanGene = mean(geneFeats,2)';
    meanControl = mean(controlFeats,2)';
    d = pdist2(meanGene,meanControl);
    genesDist = pdist2(geneFeats',meanGene);
    controlDist = pdist2(controlFeats',meanControl);
    genesStd = std(genesDist);
    controlStd = std(controlDist);
    geneDayDiff{iGeneDay}.twoMeansTest = d / (nGene*genesStd/nsamples + nControl*controlStd/nsamples);
    ...
    set(h,'Color','none');
    outFname = [mainDirname 'svm/cls_' curGeneStr '_' geneDayDiff{iGeneDay}.dayStr '_' propertyStr '.eps'];
    export_fig(outFname);
    hold off;
    close all;
end
...
fprintf(sprintf('\n ****** Confidence ********\n'));
for iGeneDay = 1 : nGeneDay   
    if isfield(geneDayDiff{iGeneDay},'svmAccuracy')
        nGene = sum(geneDayDiff{iGeneDay}.geneInds);
        nControl = sum(geneDayDiff{iGeneDay}.controlInds);
        fprintf(logger,sprintf('%s (gene = %d, control = %d): %s (%f.4))\n',geneDayDiff{iGeneDay}.geneStr,nGene,nControl,geneDayDiff{iGeneDay}.svmAccuracy,geneDayDiff{iGeneDay}.pval));
    end
end
...
for iGeneDay = 1 : nGeneDay   
    if isfield(geneDayDiff{iGeneDay},'svmAccuracy')
        curExp = curExp + 1;
        distMeanGeneToMeanControl = pdist2(geneDayDiff{iGeneDay}.diffMeanGeneToMeanControl',zeros(1,12));
        %         plot(distMeanGeneToMeanControl,geneDayDiff{iGeneDay}.pval,'o','MarkerEdgeColor','k','LineWidth',2,'MarkerSize',7);
        plot(distMeanGeneToMeanControl,geneDayDiff{iGeneDay}.twoMeansTest,'o','MarkerEdgeColor','k','LineWidth',2,'MarkerSize',7);
        distances = [distances distMeanGeneToMeanControl];
        pvals = [pvals geneDayDiff{iGeneDay}.pval];
        twoMeans = [twoMeans geneDayDiff{iGeneDay}.twoMeansTest];
        genesStr{curExp} = geneDayDiff{iGeneDay}.geneStr;        
    end
end
% plot([0,max(distances)],[0.05,0.05],'-r','LineWidth',3);
xlabel('Phenotypic change','FontSize',24);
ylabel('Normalized change','FontSize',24);
haxes = get(h,'CurrentAxes');
set(haxes,'YLim',[0,20]);
set(haxes,'FontSize',24);
set(h,'Color','none');
outFname = [mainDirname 'svm/summary_' propertyStr '.eps'];
export_fig(outFname);
hold off;
close all;

[sortedDistances,sortedInds] = sort(distances);
sortedPvals = pvals(sortedInds);
sortedTwoMeans = twoMeans(sortedInds); 
sortedGenesStr = genesStr(sortedInds);

fprintf(sprintf('\n ****** Targets ********\n'));
for iGeneDay = 1 : length(distances)   
%     fprintf(logger,sprintf('%s: %f.0, %f.4\n',sortedGenesStr{iGeneDay},sortedDistances(iGeneDay),sortedPvals(iGeneDay)));
    fprintf(logger,sprintf('%s: %f.0, %f.0\n',sortedGenesStr{iGeneDay},sortedDistances(iGeneDay),sortedTwoMeans(iGeneDay)));
end
...
% % Gene vs. pSup
% for iGeneDay = 1 : nGeneDay    
%     rmpath('/apps/MATLAB/R2014a/toolbox/stats/stats');
...
%     
%     % stupid patch!
%     if (sum(clsLabels(1:nGene) == labels(1:nGene)) + sum(clsLabels(nGene+1:end) == labels(nGene+1:end))) < ...
%             (sum(-clsLabels(1:nGene) == labels(1:nGene)) + sum(-clsLabels(nGene+1:end) == labels(nGene+1:end)))
%         clsWeights = -clsWeights;
%         clsLabels = -clsLabels;
%     end
%     
%     addpath(genpath('/apps/MATLAB/R2014a/toolbox/stats/stats'));
%     addpath(genpath('/apps/MATLAB/R2013a/toolbox/stats/stats'));
%     pval = ranksum(clsWeights(1:nGene),clsWeights(nGene+1:end));
%     geneDayDiff{iGeneDay}.pval = pval;
%     geneDayDiff{iGeneDay}.svmAccuracy = [num2str(sum(clsLabels == labels)) '/' num2str(length(labels))];
