function [cellData,dataSet] = formatMovieListTimeSeriesProcess_KRC(movieObj,nIMFs,processType,varargin)
...
for iCell = 1:nCell
    ...
    if hasNotBeenProc % If data has not been processed
        ...
        %|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
           timeSeries = EdgeVelocityPreProcessing_KRC(timeSeries,nIMFs);
        %|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
