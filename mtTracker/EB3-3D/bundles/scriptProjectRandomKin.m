...
MLPath='C:\Users\Philippe\project-local\externBetzig\analysis\adavid\smallSample\prometaphase\analysis';
movieListFileNames={'earlyAndLateML.mat'};
...
for mlIdx=1:length(aMovieListArray)
    ...
    for mIdx=1:length(ML.movieDataFile_)
        ...
        ylim(handles(mIdx),[2 8]);
        legend(handles(mIdx),{'Kin','Random'});
        title(handles(mIdx),MD.movieDataFileName_);
    end
end
