function metaphasePlateProjection(MD,varargin)
    ip = inputParser;
...
if(~isempty(p.package)&&(length(p.package.processes_)>=lpid)&&(~isempty(p.package.getProcess(lpid))))
    ...
    project1D(  MD,'FoF',refs(1,2), 'dynPoligonREF',[refs(1,2).applyBase(interpolarKin,'')], ...
...
if(~GenericPackage.processExist(p.package,lpid))
    ...
    overlayProjDetectionMovie(processProjSpindleRef,'detections', oDetectionsP1P2, ... 
      'colorIndx',arrayfun(@(d) ceil(254*mat2gray(d.xCoord(:,1)))+1, tmp.movieInfo,'unif',0), ...
...
if(~GenericPackage.processExist(p.package,lpid))
    ...
    overlayProjTracksMovie(processProj,'tracks', ([P1;P2;interpolarKin]) , ...
