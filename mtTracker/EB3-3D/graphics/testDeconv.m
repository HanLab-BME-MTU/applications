...
decvol=deconvlucy(double(volISO),double(PSF),20,0, ones(size(volISO)),100,1);
...
imshow(horzcat(mat2gray(slice), ...
        mat2gray(volISO(:,:,testSliceNBISO)), ...
        mat2gray(imresize(decslice,size(slice,1)/size(decslice,1))), ...
        mat2gray(imresize(decvol(:,:,testSliceNBISO),size(slice,1)/size(decvol(:,:,testSliceNBISO),1))))) ;
...
%% testing the same PSF with GPUDeconv (I suspect an issue with the PSF such as centering)
