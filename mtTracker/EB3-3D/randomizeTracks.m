...
switch(p.randomType)
    ...
    case 'manifoldAntispace'
        ...
        for i=1:length(randKinTracks)
            randKinTracks(i)=randomizeDenseTrackBruteForce(kinTracks(i),p.dynManifoldsCell,p.mappingDistance,p.randomDist,varargin{:});
        end
end

% saturating randomization
for tIdx=1:length(randKinTracks)
  randKin=randKinTracks(tIdx);
