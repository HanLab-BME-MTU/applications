...
% if exist([upTwo filesep 'masks'],'dir') ==7  
% masksWhole = searchFiles('.tif',[],[upTwo filesep 'masks'],0); 
% wholeCellRoiMask = logical(imread([masksWhole{1,2} filesep masksWhole{1,1}]));
% else 
% wholeCellRoiMask=logical(imread([upTwo filesep 'roiMaskSeg.tif']));
% end 
wholeCellRoiMask=logical(imread([upTwo filesep 'roiMask.tif'])); 
...
%% if do a cortical calculation is on 

     %% make small section to potentially extract the distance transform of each these points as check
    testDistTransPop =1 ;    
...
if refineDwell == 1;
    ...
    for iSubTrack = 1: length(xMatIn(:,1)) % just set entire thing up as a loop for now...
        ...
        xCoordsDwellOut(iSubTrack,:) = xMatBoundCrossIn(iSubTrack,:).*OUTDwell;
        yCoordsDwellOut(iSubTrack,:) = yMatBoundCrossIn(iSubTrack,:).*OUTDwell;
