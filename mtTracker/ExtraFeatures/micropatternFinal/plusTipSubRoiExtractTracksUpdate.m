function [projData,M]=plusTipSubRoiExtractTracksUpdate(subRoiDir,timeUnits,timeVal,varargin)
...
ip.addRequired('timeUnits',@(x)ischar(x) || isempty(x));% eventually should optional
ip.addRequired('timeVal', @(x)isscalar(x) || isempty(x)); % eventually should make optional
ip.addOptional('extractType', 'nonDirectional', @ischar); 
ip.parse(subRoiDir,timeUnits,timeVal,varargin{:});
...
% ExtractType 'subTrackStart': onlyInitiate Value of 1: Flag to bypass the GUI options and select
...
% ExtractType 'nucleation' = 1;% value of 1: Flag to bypass the GUI options and select
...
wholeCellRoiMask=imread([subRoiDir filesep 'roiMask.tif']);
...
%Extract matthe Coordinates Corresponding to the Last/First particle 
% of each track
...
% Notes:
% inIncludeRegionLast idx based on dataMatMerge as 
% pixIdxLast is the coordinate for the
% particle for each subTrack (get all values of the mask with coord 
% = to the last subtrack, if within the mask this value will be 1
% get the indices of all subtracks that are 1 (subtracks with start/
% end sites within the mask)
...
% plot all member tracks in red on top of the mask
 ...
 % 
 if boundCrossOut == 1 
     xMatIn = xMat(trckIdxBoundCrossOut,:);
     yMatIn = yMat(trckIdxBoundCrossOut,:);
     ...
     title({projNameTitle; 'Tracks Crossing Out of Region'});
...
% Get Track IDs for those Growth Sub-Tracks in the Sub-ROI of Interest
if ~isempty(strmatch(lower(timeUnits),'fraction'))
...
% Make Histograms
%trckIdxInTot = [trckIdxIn;trckIdxInPause; trckIdxInShrink]; 
%if ~isempty(trckIdxInTot)
...
rosePlotSaveDir = [subRoiDir filesep 'meta' filesep 'histograms'];
if ~isdir(rosePlotSaveDir); mkdir(rosePlotSaveDir); end
...
close(saveFig2)
    ...
    projData.subTrackAnglesIndMean = NaN(1,2); 
    projData.subTrackAnglesIndMean_INSIDE_REGION = NaN(1,2);
