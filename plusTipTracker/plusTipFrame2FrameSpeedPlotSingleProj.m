function plusTipFrame2FrameSpeedPlotSingleProj(projData,saveDir,timeRange,velLimit,roiYX,MarkerSize)
...
if nargin<1 || isempty(projData)
  ...
  [fileName,pathName]=uigetfile('*.mat','Please select projData');
...
%projData = load([char(groupList(1,2)) filesep 'meta' filesep 'projData.mat']); 
%projData = projData.projData;
...
if nargin<4 || isempty(velLimit) || isinf(velLimit)
    velLimit = [];
end
...
%for iGroup = 1:length(groupList)
 ...
 %   projData = load([char(groupList(iGroup,2)) filesep 'meta' filesep 'projData.mat']);
  %  projData = projData.projData; 
   % roiYX = load([char(groupList(iGroup,2)) filesep 'roiYXSeg.mat']); 
    %roiYX = roiYX.roiYX; 
    %mkdir([char(groupList(iGroup,2)) filesep 'SpeedPlots']); 
...
% not we might have introduced a bug here as I changed the options for the
% reclass might not be merged anymore (need to check!! MB)
...
if ~isempty(velLimit)
    ...
    m=round(nanmax(nanmax(trackVelMatrix(:,startFrame:endFrame-1))));
end
...
figure1 = figure('Position',[round(screenW*(1-movieW/screenW)/2) round(screenL*(1-movieL/screenL)/2) movieW movieL]);
...
for iFrame=startFrame:endFrame-1
    ...
    set(h,prop_name,prop_values,'MarkerSize',MarkerSize)
...
xlabel(['Speed Map of Frame to Frame Velocities, Max Speed' num2str(m) ' microns/min']); 
figure2 = figure; 
clims = [0 m]; 
imagesc(img,clims);
colorbar; 

saveas(figure1,[saveDir filesep 'Frame2Frame_Velocity_Plot','.eps'],'psc2'); 
saveas(figure1,[saveDir filesep 'Frame2Frame_Velocity_Plot','.fig']); 
saveas(figure2,[saveDir filesep 'ColorBar','.eps'],'psc2'); 
