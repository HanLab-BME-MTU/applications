function [tracksFinal,kalmanInfoLink,errFlag,diagnosticTrackLinearity] = plusTipTrackCloseGapsKalman(...
...
if isstruct(saveResults)
    save([saveResDir filesep saveResFile],'costMatrices','gapCloseParam',...
        'kalmanFunctions','tracksFinal','kalmanInfoLink','diagnosticTrackLinearity');
end
...
if ~isempty(diagnostics) && diagnostics == 1
    ...
    if ~isempty(gapInfo)
        figure('Visible','off','Name','Gap length histogram','NumberTitle','off');
