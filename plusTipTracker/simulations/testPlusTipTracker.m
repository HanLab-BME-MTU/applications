...
% Create folders for plusTip analysis
for i=1:nParameters
    for j=1:nParameters
        mkClrDir(projList(i,j).imDir);
        mkClrDir(projList(i,j).anDir);
        % Save roi containing the whole window
        roiYX = roi(i);
...
%     nCometsMean=mean(arrayfun(@(x) size(x.amp,1),movieInfoGT{1,i}));
    for j=1:nParameters
        featDir = [projList(i,j).anDir filesep 'feat'];
        mkClrDir(featDir);
...
for index=1:nParameters^2
    ...
    plusTipCometTracker(projList(i,j),timeWindow,minTrackLen,minRadius,...
...
for index=1:nParameters^2
    ...
    res=load([projList(i,j).anDir filesep 'track' filesep 'trackResults.mat']);
    ...
    [linkStats2,gapStats]=scoreLinksGapsMS(tracksPT{i,j},tracksSimMiss{i,j});
...
save([analysisFolder filesep 'analysisData.mat'],'linkFP','linkFN',...
    'fgapFP','fgapFN','bgapFP','bgapFN','nfgapGT','nbgapGT');
