function [speedCell,protSpeedCell]=quantifyMovieFlowSpeed(MD)
...
if numel(iOutput)>1 % There is protrusive actin flow speed
    for curOutput = iOutput
        sampleStr = wsProc.loadChannelOutput(iChan,curOutput);
        if contains(outName(curOutput),'Protrusion')
            protSpeedCell = sampleStr.avg(:,1:maxNumLayers,:);            
        else
            % Interpreting them
...
else %there is not protrusive actin flow speed
    protSpeedCell = [];
