...
%
%specifically adapted for DNA paint 4Edge images
...
%removes drift markers from consideration
temp =locmax2d(counts,[15,15]);
counts2 =counts;
sCounts = size(counts);
[pi,pj] = ind2sub(sCounts,find(temp >65));
for i=1:numel(pi)
    if (pi(i) > 7) & (pi(i) <sCounts(1)-7) & (pj(i) > 7) & (pj(i) <sCounts(2)-7)
        counts2(pi(i)-7:pi(i)+7,pj(i)-7:pj(i)+7)=0;
    end
end
