function [bestk,bestpp,bestmu,bestcov,bestindic,dl,countf] = mixtures4_GaussPois2(y,kmin,kmax,lambda,th,globcov,pp,mu,verbose)
%
% Syntax:
...
FirstIndex = dt>0 & dt < 10/lambda;
Norm = dt > 0;
dt(dt <= 0)=1;
TimeP = Norm.*Pois(dt);


%Probability of this event being the first of the cluster
ProbFirst = exp(-lambda*(t-1));
...
if nargin < 8 | isempty(mu)
    %Intialization is done at random but biased towards points that appear
    %early in time. The probability that the first event from an emmiter
    %has occured is 1-e^(-l*t) where t is absolute time. The probability
    %that a point is the first appearance of a dyes is e^(-l*t).
    %
    %So when intiallizing the model use a standard monte carlo acceptance
    %step to choose our intial guesses.
    
    %randomly sort the data points
    randindex = randperm(npoints);
    
    %find the probability of these points being the first point
    
%     
%     %Pick randomly monte carlo style
%     %Compares each probability to a random number between 0 and 1 and then
%     %picks the first k models where the ProbFirst is larger than that
%     %random number.
%     randindex = randindex(find(ProbFirst > random('unif',0,1,size(ProbFirst)),k));
    [Sorted,idx]=sort(ProbFirst(randindex),'descend');
    randindex = randindex(idx(1:k));
    estmu = y(:,randindex);
%     
%     randindex = randperm(npoints);
...
for i=1:k
    time_indic(i,:) = multiPoisVec(TimeP,normindic(i,:),k,ProbFirst);
...
while(k_cont)  % the outer loop will take us down from kmax to kmin components
    ...
    %sets inital values for best of inner loop
    loopdl = dl(countf);
    looppp = estpp;
    loopmu = estmu;
    loopk = k;
    ...
    while(cont)    % this inner loop is the component-wise EM algorithm with the
        ...
        while comp <= k
            ...
            clear temp
            for i=1:k
                temp = multiPoisVec(TimeP,normindic(i,:),k,ProbFirst);
...
%                 end
            ...
            %estpp(comp) = max(sum(normindic(comp,:)),0)/npoints;
        ...
        %check if this is the best cost in this round of the EM looping
        if dl(countf) < loopdl
            loopdl = dl(countf);
            looppp = estpp;
            loopmu = estmu;
        ...
        counter=counter+1;
        if (abs(deltlike/loglike(countf-1)) < th) | isnan(deltlike) | counter > 200
            ...
            %this step reverts the state to the lowest dl state over these
            %200 steps it is likely that the cost is oscilating. 
            if counter > 200
                countf = countf+1; %note this is an artifical iteration step
                dl(countf)= loopdl;
                estpp = looppp;
                estmu= loopmu;
                estcov = loopcov;
    ...
    %clears inner loop local bests
    clear loopdl loopmu loopcov loopk loopindic looppp
...
% = e^(-lt)*( t + 1/l - e^(-l)*(t+1+1/l)
    ti = 1:tmax; % Calculation from 0 so that if you consider something at t=1 it refects the probability of arriving between t=0 and t =1
    l = lamda;
    %dist = l*ti.*exp(-l*ti);
...
function y= multiPoisVec(Pois,indic,k,ProbFirst)
...
%
%instead if y == 0 and the probability that it is a first occurance is >
% 5% then the time component is set to 1
medy = median(y(y>0));
y(y<medy/100 & (ProbFirst>=0.05 | indic > 1/k))=medy;
