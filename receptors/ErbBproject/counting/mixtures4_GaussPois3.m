function [bestk,bestpp,bestmu,bestcov,bestindic,dl,countf] = mixtures4_GaussPois3(y,kmin,kmax,lambda,th,globcov,pp,mu,verbose)
%
% Syntax:
...
%     randindex = randindex(find(ProbFirst > random('unif',0,1,size(ProbFirst)),k));
    ...
    [Sorted,idx]=sort(ProbFirst,'descend');
...
[n,numpnts]=size(Pois);
t = sum(Pois.*repmat(indic',[1,numpnts]));
N=sum(indic);
if N > 0
    t=t/N;
else
...
%Handles the special case of the first point by making y a sum of
%prob(first)+prob(not first)*(prob(point is a next point))
y = ProbFirst + (1-ProbFirst).*t;
