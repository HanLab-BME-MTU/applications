classdef ColocalizationPt2CntProcess < ImageAnalysisProcess
   % A concreate class for measuring colocalization between two images
   % Anthony Vega 09/2014
        methods (Access = public)
        function obj = ColocalizationPt2CntProcess(owner,varargin)
            ...
            if nargin == 0
                ...
                super_args{1} = owner;
                super_args{2} = ColocalizationPt2CntProcess.getName;
                super_args{3} = @colocalizationPt2CntWrapper;
                if isempty(funParams)
                    funParams = ColocalizationPt2CntProcess.getDefaultParams(owner,outputDir);
                end
