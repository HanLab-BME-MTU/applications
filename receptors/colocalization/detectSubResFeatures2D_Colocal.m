function [movieInfo,exceptions,localMaxima,background,psfSigma,movieCheck] = ...
    detectSubResFeatures2D_Colocal(movieParam,detectionParam,saveResults,verbose)
...
last5start = max(numImagesRaw,1); %Tony temp edit max(numImagesRaw-4,1)
i = 0;
...
for iWindow = 1 : numIntegWindow
    ...
    for iImage = 1 : numImagesInteg(iWindow)
        ...
        imageTmp = imread([imageDir filenameBase enumString(imageIndx(iImage),:) '.tif']);
        [imageSizeX,imageSizeY] = size(imageTmp);
        ...
        if absBG
             imageTmp = imread([bgImageDir bgImageBase enumString(imageIndx(iImage),:) '.tif']);
             [bgSizeX,bgSizeY] = size(imageTmp);
...
% %             bgMeanMax = bgMeanRaw(localMax1DIndx); Tony Temp Edit, only
% using bgMeanmMaxF and no longer bgMeanMax
            ...
            if numLocalMax == 0 %if there are no local maxima
                ...
                for iMax = 1 : numLocalMax
                    cands(iMax).IBkg = bgMeanMaxF(iMax);
...
for iImage = goodImages(:)'
    ...
    %read raw image
    if hasImageDir
        imageRaw = imread([imageDir filenameBase enumString(imageIndx(iImage),:) '.tif'],1);
    else
        imageRaw = channel.loadImage(imageIndx(iImage));
    end
    imageRaw = double(imageRaw) / (2^bitDepth-1);
