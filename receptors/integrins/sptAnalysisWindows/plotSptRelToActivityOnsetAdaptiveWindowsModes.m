    function plotSptRelToActivityOnsetAdaptiveWindowsModes(particleBehavior,...
    windowDistFromEdge,compLastMode,minNP,figureName,saveLoc,convFact,...
...
for iMode = 1 : numMode
    edgeSeriesMean(:,iMode) = [dynamicSeriesBefMean(1:-minInc+1,1,iMode); diag(dynamicSeriesAftMean(-minInc+2:end,:,iMode))];
    edgeSeriesStd(:,iMode) = [dynamicSeriesBefStd(1:-minInc+1,1,iMode); diag(dynamicSeriesAftStd(-minInc+2:end,:,iMode))];
    edgeSeriesNP(:,iMode) = [dynamicSeriesBefNP(1:-minInc+1,1,iMode); diag(dynamicSeriesAftNP(-minInc+2:end,:,iMode))];
end
...
%get rid of last mode, which is the unclassified fraction
if numMode > 1
    
    staticSeriesMean = staticSeriesMean(:,:,1:end-1);
    staticSeriesStd = staticSeriesStd(:,:,1:end-1);
    staticSeriesNP = staticSeriesNP(:,:,1:end-1);
    
    dynamicSeriesBefMean = dynamicSeriesBefMean(:,:,1:end-1);
    dynamicSeriesBefStd = dynamicSeriesBefStd(:,:,1:end-1);
    dynamicSeriesBefNP = dynamicSeriesBefNP(:,:,1:end-1);
    
    dynamicSeriesAftMean = dynamicSeriesAftMean(:,:,1:end-1);
    dynamicSeriesAftStd = dynamicSeriesAftStd(:,:,1:end-1);
    dynamicSeriesAftNP = dynamicSeriesAftNP(:,:,1:end-1);
    
    edgeSeriesMean = edgeSeriesMean(:,1:end-1);
    edgeSeriesStd = edgeSeriesStd(:,1:end-1);
    edgeSeriesNP = edgeSeriesNP(:,1:end-1);
    
    if compLastMode
        staticSeriesMean = staticSeriesMean ./ repmat(sum(staticSeriesMean,3),[1 1 numMode-1]);
        dynamicSeriesBefMean = dynamicSeriesBefMean ./ repmat(sum(dynamicSeriesBefMean,3),[1 1 numMode-1]);
        dynamicSeriesAftMean = dynamicSeriesAftMean ./ repmat(sum(dynamicSeriesAftMean,3),[1 1 numMode-1]);
        edgeSeriesMean = edgeSeriesMean ./ repmat(sum(edgeSeriesMean,2),[1 numMode-1]);
    end
...
if ~isnan(yMax)
    ...
    bar((minInc:maxInc)*convFactTime,edgeSeriesMean,'stacked')
    for iMode = 1 : size(edgeSeriesMean,2)
        myErrorbar((minInc:maxInc)*convFactTime,sum(edgeSeriesMean(:,1:iMode),2),edgeSeriesSem(:,iMode))
    end
...
%     plot([0 0],[yMin yMax],'k:')
    ...
    bar((minInc:maxInc)*convFactTime,squeeze(staticSeriesMean(:,1,:)),'stacked')
    for iMode = 1 : size(staticSeriesMean,3)
        myErrorbar((minInc:maxInc)*convFactTime,sum(staticSeriesMean(:,1,1:iMode),3),staticSeriesSem(:,1,iMode))
    end
