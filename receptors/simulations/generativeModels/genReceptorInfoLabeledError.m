function receptorInfoLabeled = genReceptorInfoLabeledError(receptorInfoAll,...
    labelRatio,intensityQuantum,posError)
...
%                         labaled receptors only. It has 3 additional fields:
...
%
%Modified June 2015, Paul Blazek, to include multi-colored labeling and error.
...
%guess the size of the observation so that, with error, receptors stay
%inside the boundaries
observeSideLen = ceil(max(receptorTraj(:)));
...
for lRindx=1:numSamples
    ...
    if labelRatio(lRindx,1) < 1
        ...
        for color = 1:numColors
            ...
            receptorTrajLabeled = receptorTrajLabeled + randn(size(receptorTrajLabeled))*posError;
            
            %keep within boundaries
            correctionBoundaryLow = min(receptorTrajLabeled,0);
            receptorTrajLabeled = receptorTrajLabeled - 2 * correctionBoundaryLow;
            correctionBoundaryUp = max(receptorTrajLabeled - observeSideLen,0);
            receptorTrajLabeled = receptorTrajLabeled - 2 * correctionBoundaryUp;
