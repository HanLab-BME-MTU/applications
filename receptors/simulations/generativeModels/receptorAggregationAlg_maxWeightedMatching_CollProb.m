function [cluster2receptor,receptor2cluster,clusterSize,receptPositions] = ...
    receptorAggregationAlg_maxWeightedMatching_CollProb(receptPositionsPrev,...
    associationDistVals,aggregationProbVec,aggregationProb,collisionProbVals,...
    receptor2clusterPrev,cluster2receptorPrev,clusterSizePrev)
%RECEPTAGGREGATIONALG_MAXWEIGHTEDMATCHING_COLLPROB performs receptor-to-receptor
...
%   were attempted by adding two types of collision probabilities
%   implemented in this function: the linear and arc models. Refer to 
%   summary from 07/11/14 for more details.
%   
%   INPUT:
...
%       aggregationDistCorr:    the corrected distance threshold
%
...
%
%       collisionProbVals:      a struct with the following fields used in
%                               calculating collision probability via a
%                               linear model
...
%
    ...
    if (any(nodeFlag))
        %052714 - aggregationDist is now passed in a struct
        aggregationDist = associationDistVals.associationDistCorr;
        ...
        if (~isempty(aggregNodeIndx_aggregDist))
            ...
            for aggregNodeIter=1:numNodesAggreg                
                ...
                if (~isempty(partnerNodeID))
                    ...
                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    %052714
                    %Next determine collision probabilities based on the
                    %pairwise distances. This value is 1 for pw distances
                    %<= the user defined associationDist. For those > the
                    %associationDist but less than associationDistCorr, use
                    %the linear model to determine a collision probability.
                    
                    collisionProb = NaN(length(partnerNodeID),1);
                    %Get pair-wise distances between the current node and
                    %its potential partners
                    currPWDist = (aggregNodePWDist(partnerNodeIndx,...
                        aggregNodeIndx_aggregDist(aggregNodeIter))');
                    %Set those < associationDist to 1
                    collisionProb(currPWDist < associationDistVals.associationDist) = 1;
                    %Find index of those with pwDist >= associationDist but
                    %< associationDistCorr
                    tempPWDistIndx = find ((currPWDist >= associationDistVals.associationDist) &...
                        currPWDist < associationDistVals.associationDistCorr);
                    
                    if (~isempty(collisionProbVals))
                        %Set those >= associationDist but < associationDistCorr
                        %via the linear model                    
                        collisionProb(tempPWDistIndx) = collisionProbVals.m*currPWDist(tempPWDistIndx) +...
                            collisionProbVals.b;
                    else
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        %060414
                        %Implementing collision probability via the arc
                        %model. To use this, collisionProbVals must be set
                        %to [] in main simulation. Only attempt this
                        %if there are pairwise distances that fall between
                        %associationDist and associationDistCorr.
                        
                        if (~isempty(tempPWDistIndx))
                            %Calculate rmsd, which is half of the corrected
                            %association distance. Setup vector for arrayfun -
                            %note column-wishe
                            rmsdVec = repmat(associationDistVals.associationDistCorr./2,...
                                1,length(tempPWDistIndx));
                            %Get probabilities via arc model
                            tempCollProb_arc = arrayfun(@calcCollisionProb_arcModel,...
                                rmsdVec,currPWDist(tempPWDistIndx));
                            %Make the appropriate assignment
                            collisionProb(tempPWDistIndx) = tempCollProb_arc;
                        end
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    end
                    ...
                    assocFlag = (rand(length(partnerNodeID),1) <...
                        aggregationProb(finalSizes).*collisionProb );
                    
                    
                    %Remove from the list partner nodes not associating
                    partnerNodeID(assocFlag == 0) = [];
                    partnerNodeIndx(assocFlag == 0) = [];
                    
                end %if associating partner nodes exist
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                    
                ...
                if (numPartners > 0)
                    ...
                    clear tempEdges tempWeights partnerNodeID partnerNodeIndx 
                    clear numPartners tempPWDistIndx currPWDist
                    clear collisionProb assocFlag rmsdVec tempCollProb_arc
                end
