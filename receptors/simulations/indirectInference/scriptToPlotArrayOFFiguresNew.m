...
%     for rDTIndx = 1 : length(rDDir)
        ...
        for rowIndex=[1,1+lengthLr,1+2*lengthLr,1+3*lengthLr,1+4*lengthLr,1+5*lengthLr,1+6*lengthLr,1+7*lengthLr]
            temp= load([resultsDirectory,filesep,filesep,rDtarget{1},dRDirT{1},aPtarget{1},lRtarget{1},filesep,'pMatrix.mat']);
...
% % 
 for rDindx=1:length(rDDir)
      temp= load([resultsDirectorySR,filesep,rDtarget{1},dRDirT{1},aPtarget{1},lRtargetSR{1},filesep,'pMatrix_',lRtargetSR{1},'.mat']);
      pMatrix=temp.pMatrix;
      pMatrix(pMatrix<0.05)=0.05;
       subplot(8,7,7*rDindx);
       ax = gca;
       load('MyColormaps','mycmap')
       colormap(ax,mycmap)
