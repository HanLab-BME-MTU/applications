function plotTracksPart(tracksDivided,diffAnalysisRes,timeRange,...
...
assert(isstruct(tracksDivided),'Input argument tracksPart is not a structure')
numTracks = length(tracksDivided);
tmp = vertcat(tracksDivided.seqOfEvents);
...
for i = 1 : numTracks
    numSegments(i) = size(tracksDivided(i).tracksCoordAmpCG,1);
end
...
for i = 1 : numTracks
    startTime = tracksDivided(i).seqOfEvents(1,1);
    endTime   = max(tracksDivided(i).seqOfEvents(:,1));
    trackedFeatureInfo(trackStartRow(i):trackStartRow(i)+...
        numSegments(i)-1,8*(startTime-1)+1:8*endTime) = ...
        tracksDivided(i).tracksCoordAmpCG;
    if i == 14
    end
    trackInside(trackStartRow(i):trackStartRow(i)+...
        numSegments(i)-1) = tracksDivided(i).isInside;
end   
...
for i = 1 : numTrackSegments
    ...
    if trackInside(i) == 1 % If track is inside
        % Plot a thick line
        plot(tracksXP(obsAvail,i),tracksYP(obsAvail,i),'k:','LineWidth',1);
...
for iTrack = 1 : numTracks
    ...
    indxMerge = (find(seqOfEvents(:,2) == 2 & ~isnan(seqOfEvents(:,4)) ...
        & seqOfEvents(:,1) > timeRange(1) & seqOfEvents(:,1) <= timeRange(2)))';
    indxEnter = (find(seqOfEvents(:,2) == 3) ...
        & seqOfEvents(:,1) > timeRange(1) & seqOfEvents(:,1) <= timeRange(2))';
    indxExit = (find(seqOfEvents(:,2) == 4) ...
        & seqOfEvents(:,1) > timeRange(1) & seqOfEvents(:,1) <= timeRange(2))';
