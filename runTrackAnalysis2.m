...
function runTrackAnalysis2(data, varargin)
...
nSingleTracks = numel(singleTrackIdx);
gapMap = zeros(nSingleTracks, data.movieLength);

for i = 1:nSingleTracks
    ti = singleTrackIdx(i);
    gapMap(i, tracks(ti).start:tracks(ti).end) = tracks(ti).gapVect;
end

trackStarts = [tracks(singleTrackIdx).start];
trackEnds = [tracks(singleTrackIdx).end];

fullTracks = [tracks(singleTrackIdx).visibility]==1;
...
for f = 1:data.movieLength
    ...
    for ch = 1:nCh
        ...
        for ki = 1:numel(currentGapsIdx)
            k = singleTrackIdx(currentGapsIdx(ki));
        ...
        for ki = 1:length(currentBufferIdx)
            k = singleTrackIdx(currentBufferIdx(ki));
