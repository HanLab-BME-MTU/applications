...
[traj1,errFlag] = simSetarma(0,[],0,arOrder,0,[-0.5 0.1 0.2 -0.7 -0.4],[],0.1,trajLength,[0 0 0 0 0]);
if errFlag
    return
end
[arParam,varCovMat,residuals,noiseSigma,errFlag] = arlsestim0(traj1,arOrder);

for i = 1:1
    
    traj2 = traj1; %copy traj1 into traj2
    ...
    actMiss = length(find(isnan(traj2))); %find number of points actually deleted 
    ...
    while actMiss < missing %repeat loop until required number of points is deleted
        ...
        traj2(indx) = NaN;
        actMiss = length(find(isnan(traj2)));
    end
    ...
    [arParam0(i,:),varCovMat0(:,:,i),residuals0(:,i),noiseSigma0(i),trajP0,errFlag] = arlsIterEstim(traj2,arOrder,0.001,0);
    if errFlag
        return
    end
    [arParam1(i,:),varCovMat1(:,:,i),residuals1(:,i),noiseSigma1(i),trajP1,errFlag] = arlsIterEstim(traj2,arOrder,0.001,1);
    if errFlag
        return
    end
    [arParam2(i,:),varCovMat2(:,:,i),residuals2(:,i),noiseSigma2(i),trajP2,errFlag] = arlsIterEstim(traj2,arOrder,0.001,2);
    if errFlag
        return
    end
    [arParam3(i,:),varCovMat3(:,:,i),residuals3(:,i),noiseSigma3(i),trajP3,errFlag] = arlsIterEstim(traj2,arOrder,0.001,3);
    if errFlag
        return
    end
    [arParam4(i,:),varCovMat4(:,:,i),residuals4(:,i),noiseSigma4(i),trajP4,errFlag] = arlsIterEstim(traj2,arOrder,0.001,4);
    if errFlag
        return
    end
    [arParam5(i,:),varCovMat5(:,:,i),residuals5(:,i),noiseSigma5(i),trajP5,errFlag] = arlsIterEstim(traj2,arOrder,0.001,5);
    if errFlag
        return
    end
    
    [trajP00,errFlag] = missPointARPred(traj2,arParam00,0);
    if errFlag
        return
    end
    [trajP01,errFlag] = missPointARPred(traj2,arParam00,1);
    if errFlag
        return
    end
    [trajP02,errFlag] = missPointARPred(traj2,arParam00,2);
    if errFlag
        return
    end
    [trajP03,errFlag] = missPointARPred(traj2,arParam00,3);
    if errFlag
        return
    end
    [trajP04,errFlag] = missPointARPred(traj2,arParam00,4);
    if errFlag
        return
    end
    [trajP05,errFlag] = missPointARPred(traj2,arParam00,5);
    if errFlag
        return
    end
