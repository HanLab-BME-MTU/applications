function [frameInfo] = spotDetection(sourcePath)
%
% Performs detection of local intensity clusters through denoising by iterative
% filtering from significant coefficients:
...
% Process inputs
%===================================================
if nargin==0 || isempty(sourcePath)
...
for k = 1%:nFrames
    ...
    imgDenoised = imgDenoised - min(imgDenoised(:));
    imgDenoised = imgDenoised/max(imgDenoised(:))*(max(img(:))-min(img(:)));
    ...
    for n = 1:nComp
        ...
        elseif nMaxima(n)==0 % no maximum was detected for this cluster
            ...
            xmax2{n} = xm(mindist > dthreshold);
            ymax2{n} = ym(mindist > dthreshold);
            nSecMax = length(xm);
            nMaxima(n) = nSecMax+1;
            
            % split area
            area2{n} = area(n)*ones(1,nSecMax)/nMaxima(n);
            area(n) = area(n)/nMaxima(n);
            labels2{n} = labels(n)*ones(1,nSecMax);
            
            %intensity values
            totalInt2{n} = totalInt(n)*ones(1,nSecMax)/nMaxima(n);
    ...
    frameInfo(k).xCoord(:,1) = frameInfo(k).xmax;
    frameInfo(k).yCoord(:,1) = frameInfo(k).ymax;
