...
prompt = {...
    ...
    'nearestNeighborFilteringEnabled',...
    'kThNearestNeighbor',...
    'kThNearestNeighborDistanceTH',...
    };
...
defaultanswer = {...
    ...
    num2str(data.currentConfig.nearestNeighborFilteringEnabled),...
    num2str(data.currentConfig.kThNearestNeighbor),...
    num2str(data.currentConfig.kThNearestNeighborDistanceTH),...
    };
options.Resize = 'on';
options.WindowStyle = 'normal';
answer = inputdlg(prompt,name,[ones(18,1),ones(18,1)*50],defaultanswer,options);
if ~isempty(answer)
    ...
    data.currentConfig.nearestNeighborFilteringEnabled = str2double(answer{16});
    data.currentConfig.kThNearestNeighbor  = str2double(answer{17});
    data.currentConfig.kThNearestNeighborDistanceTH  = str2double(answer{18});
end
guidata(gcf,data);
...
if ~isempty(answer)
    ...
    data.currentConfig.roiPosition(1) = str2double(answer{5}(1:delimiterPos(1)-1));
    data.currentConfig.roiPosition(2) = str2double(answer{5}(delimiterPos(1)+1:delimiterPos(2)-1));
    data.currentConfig.roiPosition(3) = str2double(answer{5}(delimiterPos(2)+1:end));
    delimiterPos = strfind(answer{6},',');
    data.currentConfig.roiSize(1) = str2double(answer{6}(1:delimiterPos(1)-1));
    data.currentConfig.roiSize(2) = str2double(answer{6}(delimiterPos(1)+1:delimiterPos(2)-1));
    data.currentConfig.roiSize(3) = str2double(answer{6}(delimiterPos(2)+1:end));
    data.currentConfig.initialEdgeRadiusGeom = str2double(answer{7});
    data.currentConfig.initialEdgeRadius = str2double(answer{8});
    data.currentConfig.filterLength = str2double(answer{9});
    data.currentConfig.angularSampling = str2double(answer{10});
    data.currentConfig.displayEnabled = str2double(answer{11});
    data.currentConfig.snapshotsEnabled = str2double(answer{12});
    data.currentConfig.fitMethod = str2double(answer{13});
    data.currentConfig.edgeWidthInitFree = str2double(answer{14});
    data.currentConfig.betaVar = str2double(answer{15});
    data.currentConfig.modeVar = str2double(answer{16});
end
guidata(gcf,data);
