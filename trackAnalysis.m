...
for k = 1:nTracks
    ...
    if tracks(k).valid
        ...
        % gaps
        %===============
        trackIdx = find(isnan(tracks(k).A));
        frameIdx = trackIdx + tracks(k).start - 1;
        
        % linear interpolation of background values
        tracks(k).c(trackIdx) = interp1(find(~isnan(tracks(k).c)), tracks(k).c(~isnan(tracks(k).c)), trackIdx);
        
        for g = 1:length(trackIdx)
            frame = double(imread([data.source frameList(frameIdx(g)).name]));
            
            xi = round(tracks(k).x(trackIdx(g)));
            yi = round(tracks(k).y(trackIdx(g)));
            window = frame(yi-w:yi+w, xi-w:xi+w);
            [p] = fitGaussian2D(window, [tracks(k).x(trackIdx(g))-xi tracks(k).y(trackIdx(g))-yi max(window(:))-tracks(k).c(trackIdx(g)) sigma tracks(k).c(trackIdx(g))], 'A');
            tracks(k).A(trackIdx(g)) = p(3);
        end
    end
