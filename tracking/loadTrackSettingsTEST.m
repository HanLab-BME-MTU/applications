...
function trackSettings = loadTrackSettingsTEST(varargin)
...
ip.addParamValue('LinearMotion', 1);
ip.addParamValue('IntensityConstraint', [0.8 1.2]);
ip.parse(varargin{:});
...
if all(intensityConstraint == [0 Inf]);
    gapCostFun = 'costMatLinearMotionCloseGaps2';
else
    gapCostFun = 'costMatLinearMotionCloseGapsCCP';
end
...
costMatrices(1).parameters.linearMotion = linearMotion; %use linear motion Kalman filter.
costMatrices(1).parameters.minSearchRadius = radius(1); %minimum allowed search radius. The search radius is calculated on the spot in the code given a feature's motion parameters. If it happens to be smaller than this minimum, it will be increased to the minimum.
costMatrices(1).parameters.maxSearchRadius = radius(2); %maximum allowed search radius. Again, if a feature's calculated search radius is larger than this maximum, it will be reduced to this maximum.
...
costMatrices(2).funcName = gapCostFun;
costMatrices(2).parameters.linearMotion = linearMotion; %use linear motion Kalman filter.
costMatrices(2).parameters.minSearchRadius = radius(1); %minimum allowed search radius.
costMatrices(2).parameters.maxSearchRadius = radius(2); %maximum allowed search radius.
...
costMatrices(2).parameters.ampRatioLimit = intensityConstraint; %for merging and splitting. Minimum and maximum ratios between the intensity of a feature after merging/before splitting and the sum of the intensities of the 2 features that merge/split.
